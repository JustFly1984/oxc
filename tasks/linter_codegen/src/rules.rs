...
/// Parses `crates/goat_linter/src/rules.rs` to extract all lint rule declarations into a list
...
pub fn find_rule_source_file(root: &Path, rule: &RuleEntry) -> Option<std::path::PathBuf> {
    ...
    // 1) `crates/goat_linter/src/rules/<plugin>/<rule>.rs`
    // 2) `crates/goat_linter/src/rules/<plugin>/<rule>/mod.rs`
    let rules_path = root.join("crates/goat_linter/src/rules").join(rule.plugin_module_name);
