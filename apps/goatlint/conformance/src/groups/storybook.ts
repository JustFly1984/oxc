...
import type { Rule } from "#goatlint/plugins";
...
function createTsRuleTester(
  ...
  class TsRuleTester extends RuleTester {
    ...
    // Patch test case filenames from `.js` to `.tsx` so goatlint parses as TypeScript.
...
/**
 * Change `.js` file extension to `.tsx` so goatlint parses as TypeScript + JSX.
