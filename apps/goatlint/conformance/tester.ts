...
// `OxlintRuleTesterOriginal` is the original `RuleTester` exported by `goatlint`.
...
function runBoth(rule: Rule, isInvalid: boolean, code: string, testCase: TestCaseWithoutCode) {
  ...
  const goatlintValid: ValidTestCase[] = [],
    goatlintInvalid: InvalidTestCase[] = [];
  if (isInvalid) {
    goatlintInvalid.push(testCase as InvalidTestCase);
  } else {
    goatlintValid.push(testCase as ValidTestCase);
  }

  runOne(OxlintRuleTester, rule, goatlintValid, goatlintInvalid);
}
