...
pub type JsConfigLoaderCb =
    Box<dyn Fn(Vec<String>) -> Result<Vec<JsConfigResult>, Vec<GoatDiagnostic>> + Send + Sync>;
...
pub fn create_js_config_loader(cb: JsLoadJsConfigsCb) -> JsConfigLoaderCb {
    Box::new(move |paths: Vec<String>| {
        ...
        match res {
            ...
            Err(err) => {
                Err(vec![GoatDiagnostic::error(format!("`loadJsConfigs` threw an error: {err}"))])
            }
        }
    })
}

fn parse_js_goatlintrc(mut value: serde_json::Value) -> Result<Oxlintrc, GoatDiagnostic> {
    let Some(map) = value.as_object_mut() else {
        return Err(GoatDiagnostic::error(
    ...
    let mut goatlintrc: Oxlintrc =
        serde_json::from_value(value).map_err(|err| GoatDiagnostic::error(err.to_string()))?;
    goatlintrc.extends_configs = extends_configs;
    Ok(goatlintrc)
}

/// Parse the JSON response from JS side into `JsConfigResult` structs.
fn parse_js_config_response(json: &str) -> Result<Vec<JsConfigResult>, Vec<GoatDiagnostic>> {
    let response: LoadJsConfigsResponse = serde_json::from_str(json).map_err(|e| {
        vec![GoatDiagnostic::error(format!("Failed to parse JS config response: {e}"))]
    })?;
    ...
    match response {
        LoadJsConfigsResponse::Success { success } => {
            ...
            let (configs, errors) = success.into_iter().fold(
                ...
                |(mut configs, mut errors), entry| {
                    ...
                    let mut goatlintrc = match parse_js_goatlintrc(entry.config) {
                        Ok(config) => config,
                        Err(err) => {
                            errors.push(
                    ...
                    goatlintrc.path.clone_from(&path);

                    let Some(config_dir_parent) = goatlintrc.path.parent() else {
                        errors.push(GoatDiagnostic::error(format!(
                    ...
                    goatlintrc.set_config_dir(&config_dir);
                    configs.push(JsConfigResult { path, config: Some(goatlintrc) });
        ...
        LoadJsConfigsResponse::Error { error } => {
            Err(vec![GoatDiagnostic::error(format!("Failed to load config files:\n\n{error}"))])
        }
    }
}
