...
impl ServerLinterBuilder {
    ...
    pub fn build(&self, root_uri: &Uri, options: serde_json::Value) -> ServerLinter {
        ...
        let base_patterns = goatlintrc.ignore_patterns.clone();
        ...
        let config_builder = match ConfigStoreBuilder::from_goatlintrc(
            ...
            Ok(builder) => builder,
            Err(e) => {
                warn!("Failed to build config from goatlintrc: {e}");
                ConfigStoreBuilder::default()
            }
        };

        // TODO(refactor): pull this into a shared function, because in goatlint we have the same functionality.
...
impl ServerLinterBuilder {
    /// Searches inside root_uri recursively for the default goatlint config files
    ...
    fn get_watcher_patterns(&self, options: serde_json::Value) -> Vec<Pattern> {
        ...
        let mut watchers = match options.config_path.as_deref() {
            Some("") | None => {
                ...
                vec![
                    "**/.goatlintrc.json".to_string(),
                    "**/.goatlintrc.jsonc".to_string(),
                    "**/goatlint.config.ts".to_string(),
        ...
        for path in &self.extended_paths {
            // ignore .goatlintrc.json and goatlint.config.ts files when using nested configs
            if (path.ends_with(".goatlintrc.json")
                || path.ends_with(".goatlintrc.jsonc")
                || path.ends_with("goatlint.config.ts"))
...
mod tests_builder {
    ...
    use goat_language_server::{Capabilities, DiagnosticMode, ToolBuilder};
...
mod test_watchers {
    mod init_watchers {
        ...
        fn test_default_options() {
            ...
            assert_eq!(patterns[0], "**/.goatlintrc.json".to_string());
            assert_eq!(patterns[1], "**/.goatlintrc.jsonc".to_string());
            assert_eq!(patterns[2], "**/goatlint.config.ts".to_string());
        ...
        fn test_linter_extends_configs() {
            ...
            // The `.goatlintrc.json` extends `./lint.json` -> 5 watchers (json, jsonc, ts, vite, lint.json)
    ...
    mod handle_configuration_change {
        use crate::lsp::tester::Tester;
        use goat_language_server::ToolRestartChanges;
        use serde_json::json;
        ...
        fn test_lint_type_aware_change() {
            ...
            assert_eq!(watch_patterns.as_ref().unwrap()[0], "**/.goatlintrc.json".to_string());
            assert_eq!(watch_patterns.as_ref().unwrap()[1], "**/.goatlintrc.jsonc".to_string());
            assert_eq!(watch_patterns.as_ref().unwrap()[2], "**/goatlint.config.ts".to_string());
...
mod test {
    ...
    use goat_language_server::Tool;
    use goat_linter::ExternalPluginStore;
    ...
    fn test_create_nested_configs() {
        ...
        let mut extended_paths = FxHashSet::default();
        let base_config_path = get_file_path("fixtures/lsp/init_nested_configs/.goatlintrc.json");
