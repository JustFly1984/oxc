...
use goat_language_server::{DiagnosticResult, TextDocument, Tool, ToolRestartChanges};
...
fn get_snapshot_safe_uri(uri: &Uri) -> String {
    ...
    let end = safe_uri.find("goatlint").expect("goatlint not found in URI");
    safe_uri.replace_range(start + "file://".len()..end + "goatlint".len(), "<variable>");
    safe_uri
}
            ...
            infos
                ...
                .map(|(i, info)| {
                    ...
                    write!(result, "related_information[{}].message: {:?}", i, info.message)
                        .unwrap();
                    // replace everything between `file://` and `goat_language_server` with `<variable>`, to avoid
...
impl Tester<'_> {
    ...
    /// Given a relative file path (relative to `goat_language_server` crate root), run the linter
        ...
        for relative_file_path in relative_file_paths {
            ...
            let reports = FileResult {
                diagnostic: linter.run_diagnostic(&TextDocument::new(
                    &uri,
                    goat_language_server::LanguageId::default(),
                    None,
                )),
