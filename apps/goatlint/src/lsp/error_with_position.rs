...
use goat_data_structures::rope::{Rope, get_line_column};
use goat_diagnostics::{OxcCode, Severity};
use goat_linter::{
...
pub fn create_unused_directives_report(
        ...
        match unused_comment.r#type {
            RuleCommentType::All => {
                reports.push(build_unused_disable_diagnostic_report(
                    "Unused goatlint-disable directive (no problems were reported).".to_string(),
                    span,
                    severity,
                    source_text,
                    rope,
            ...
            RuleCommentType::Single(rules) => {
                for rule in rules {
                    reports.push(build_unused_disable_diagnostic_report(
                        format!(
                            "Unused goatlint-disable directive (no problems were reported from {}).",
                            rule.rule_name
                        ),
                        rule.name_span,
                        severity,
    ...
    for (rule_name, span) in unused_enable {
        let message = if let Some(rule_name) = rule_name {
            format!(
                "Unused goatlint-enable directive (no matching goatlint-disable directives were found for {rule_name})."
            )
        } else {
            "Unused goatlint-enable directive (no matching goatlint-disable directives were found)."
                .to_string()
        };
        reports.push(build_unused_disable_diagnostic_report(
            message,
...
/// Counter part of `goat_linter::*::plugin_name_to_prefix`.
...
fn disable_for_this_line(
    ...
    FixedContent {
        ...
        code: format!(
            "{content_prefix}{whitespace_string}// goatlint-disable-next-line {rule_name}\n"
        ),
...
fn disable_for_this_section(
    ...
    let comment = format!("// goatlint-disable {rule_name}\n");
...
mod test {
    ...
    fn disable_for_section_js_file() {
        ...
        assert_eq!(fix.code, "// goatlint-disable no-console\n");
    ...
    fn disable_for_section_mid_line() {
        ...
        assert_eq!(fix.code, "\n// goatlint-disable no-unused-vars\n");
        assert_eq!(fix.range.start.line, 0);
    ...
    fn disable_for_this_line_single_line() {
        ...
        assert_eq!(fix.code, "// goatlint-disable-next-line no-console\n");
        assert_eq!(fix.range.start.line, 0);
        assert_eq!(fix.range.start.character, 0);
    }
