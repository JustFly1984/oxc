...
// `tasks/website` depends on `goatlint` as a normal library, which causes linker errors if NAPI is enabled.
#[cfg(feature = "napi")]
mod js_config;
#[cfg(feature = "napi")]
mod run;
...
const DEFAULT_OXLINTRC_NAME: &str = ".goatlintrc.json";
const DEFAULT_JSONC_OXLINTRC_NAME: &str = ".goatlintrc.jsonc";
const DEFAULT_TS_OXLINTRC_NAME: &str = "goatlint.config.ts";
/// Vite config file that may contain goatlint config under a `.lint` field.
const VITE_CONFIG_NAME: &str = "vite.config.ts";

/// Return a JSON blob containing metadata for all available goatlint rules.
///
