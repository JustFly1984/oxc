...
use bpaf::Bpaf;
use goat_linter::{AllowWarnDeny, FixKind, LintPlugins};
...
pub struct BasicOptions {
    ...
    ///
    /// If not provided, Oxlint will look for a `.goatlintrc.json`, `.goatlintrc.jsonc`, or `goatlint.config.ts` file in the current working directory.
    #[bpaf(long, short, argument("./.goatlintrc.json"))]
    ...
    /// Initialize goatlint configuration with default values
    #[bpaf(switch, hide_usage)]
    pub init: bool,
}
...
pub enum ReportUnusedDirectives {
    WithoutSeverity(
        /// Report directive comments like `// goatlint-disable-line`, when no errors would have been reported on that line anyway
