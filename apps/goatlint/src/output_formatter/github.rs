...
fn format_github(diagnostic: &Error) -> String {
    ...
    let title = rule_id.map_or(Cow::Borrowed("goatlint"), Cow::Owned);
...
mod test {
    ...
    fn reporter_error() {
        let mut reporter = GithubReporter;
        let error = GoatDiagnostic::warn("error message")
        ...
        assert_eq!(
            result.unwrap(),
            "::warning file=file%3A//test.ts,line=1,endLine=1,col=1,endColumn=9,title=goatlint::error message\n"
        );
    }
    ...
    fn reporter_does_not_use_minified_fallback_for_long_annotations() {
        ...
        let diagnostic = GoatDiagnostic::warn("error message")
        ...
        assert!(output.contains("title=goatlint::error message"));
