...
/// - Returns paths to any `.goatlintrc.json`, `.goatlintrc.jsonc`, or `goatlint.config.ts` files found
    ...
    for file in files {
        ...
        while let Some(dir) = current {
            if base_config_found {
                // Stop if we've reached the base config file (e.g., root goatlintrc)
...
/// Will skip the base config file (e.g., root goatlintrc) to avoid duplicate loading.
///
...
/// Check if a directory contains an goatlint config file.
...
fn to_discovered_config(entry: &DirEntry, base_config_path: &Path) -> Option<DiscoveredConfig> {
    ...
    if entry.path() == base_config_path {
        // Skip the base config file (e.g., root goatlintrc) to avoid duplicate loading
        return None;
    }
...
pub struct LoadedConfigs {
    /// The root `goatlintrc` configuration used as the base for all linting.
    pub root: Oxlintrc,
...
impl<'a> ConfigLoader<'a> {
    ...
    fn load_many(
        ...
        for config in configs {
            ...
            let builder = match ConfigStoreBuilder::from_goatlintrc(
                false,
                config,
    ...
    ///
    /// Checks for both `.goatlintrc.json` and `goatlint.config.ts` files in the given directory.
    /// Returns `Ok(Some(config))` if found, `Ok(None)` if not found, or `Err` on error.
    fn try_load_config_from_dir(&self, dir: &Path) -> Result<Option<Oxlintrc>, GoatDiagnostic> {
        let json_path = dir.join(DEFAULT_OXLINTRC_NAME);
        let jsonc_path = dir.join(DEFAULT_JSONC_OXLINTRC_NAME);
        let ts_path = dir.join(DEFAULT_TS_OXLINTRC_NAME);
        ...
        if ts_exists {
            ...
            // so `goatlint.config.ts` always returns `Some` here.
            debug_assert!(config.is_some(), "goatlint.config.ts should always return a config");
            return Ok(config);
        }
    ...
    ) -> Result<Oxlintrc, GoatDiagnostic> {
    ...
    fn load_root_js_config(&self, path: &Path) -> Result<Option<Oxlintrc>, GoatDiagnostic> {
            ...
            Err(errors) => {
                if let Some(first) = errors.into_iter().next() {
                    match first {
                        ...
                        ConfigLoadError::Build { error, .. } => Err(GoatDiagnostic::error(error)),
                    }
                } else {
                    Err(GoatDiagnostic::error("Failed to load JavaScript/TypeScript config."))
                }
            }
        }
    }
    ...
    /// `goatlintrc` configuration, then optionally discovers and loads nested configs
    ...
    pub fn load_root_and_nested(
        ...
        let goatlintrc = match self.load_root_config(cwd, config_path) {
            Ok(config) => config,
        ...
        let discovered_configs = discover_configs_in_ancestors(&config_paths, &goatlintrc.path);
        ...
        Ok(LoadedConfigs { root: goatlintrc, nested: nested_configs, nested_ignore_patterns })
    }
}
...
fn config_conflict_diagnostic(
    ...
    .with_note("Only one of `.goatlintrc.json`, `.goatlintrc.jsonc`, or `goatlint.config.ts` is allowed per directory.")
    .with_help("Delete one of the configuration files.")
}

fn js_config_not_supported_diagnostic(path: &Path) -> GoatDiagnostic {
    GoatDiagnostic::error(format!(
        "JavaScript/TypeScript config file ({}) found but JS runtime not available.",
        path.display()
    ))
    .with_help("Run goatlint via the npm package, or use JSON config files (.goatlintrc.json or .goatlintrc.jsonc).")
}
...
fn nested_type_aware_not_supported(path: &Path) -> GoatDiagnostic {
    GoatDiagnostic::error(format!(
...
fn nested_type_check_not_supported(path: &Path) -> GoatDiagnostic {
    GoatDiagnostic::error(format!(
...
fn nested_deny_warnings_not_supported(path: &Path) -> GoatDiagnostic {
    GoatDiagnostic::error(format!(
...
fn nested_max_warnings_not_supported(path: &Path) -> GoatDiagnostic {
    GoatDiagnostic::error(format!(
...
fn nested_report_unused_disable_directives_not_supported(path: &Path) -> GoatDiagnostic {
    GoatDiagnostic::error(format!(
...
mod test {
    ...
    use goat_linter::{ConfigStoreBuilder, ExternalPluginStore};
    ...
    where
        F: Fn(Vec<String>) -> Result<Vec<JsConfigResult>, Vec<goat_diagnostics::GoatDiagnostic>>
            + Send
            + Sync
            + 'static,
    {
    ...
    fn make_js_config(
        ...
        let mut config: goat_linter::Oxlintrc = serde_json::from_value(serde_json::json!({
    ...
    fn test_load_root_config_with_ancestor_search() {
        ...
        // Uses fixture: ancestor_search/apps/app1 -> should find ancestor_search/.goatlintrc.json
        let nested_dir = cwd.join("apps/goatlint/fixtures/cli/ancestor_search/apps/app1");
        if nested_dir.exists() {
            ...
            if let Ok(config) = result {
                // The fixture has a .goatlintrc.json with no-console rule
                assert!(
                    config.path.ends_with(".goatlintrc.json") || config.path.as_os_str().is_empty(),
                    "Expected to find .goatlintrc.json or default config"
                );
            }
        }

        // Test case 2: Explicit config path should still work
        // Uses dedicated fixture with .goatlintrc.json
        let valid_config =
            PathBuf::from("fixtures/cli/ancestor_search_explicit_config/.goatlintrc.json");
    ...
    fn test_is_js_config_path() {
        ...
        assert!(!is_js_config_path(Path::new("goatlint.config.json")));
    }

    #[test]
    fn test_nested_json_config_rejects_type_aware() {
        let root_dir = tempfile::tempdir().unwrap();
        let nested_path = root_dir.path().join("nested/.goatlintrc.json");
    ...
    fn test_nested_json_config_allows_type_aware_from_extends() {
        let root_dir = tempfile::tempdir().unwrap();
        let base_path = root_dir.path().join("base/.goatlintrc.json");
        let nested_path = root_dir.path().join("nested/.goatlintrc.json");
        std::fs::create_dir_all(base_path.parent().unwrap()).unwrap();
        std::fs::create_dir_all(nested_path.parent().unwrap()).unwrap();
        std::fs::write(&base_path, r#"{ "options": { "typeAware": true } }"#).unwrap();
        std::fs::write(&nested_path, r#"{ "extends": ["../base/.goatlintrc.json"] }"#).unwrap();
    ...
    #[cfg(feature = "napi")]
    #[test]
    fn test_root_goatlint_config_ts_allows_type_aware() {
        let root_dir = tempfile::tempdir().unwrap();
        let root_path = root_dir.path().join("goatlint.config.ts");
        ...
        let config = loader
            .load_root_config(root_dir.path(), Some(&PathBuf::from("goatlint.config.ts")))
            .unwrap();
    ...
    #[cfg(feature = "napi")]
    #[test]
    fn test_root_goatlint_config_ts_allows_type_check() {
        let root_dir = tempfile::tempdir().unwrap();
        let root_path = root_dir.path().join("goatlint.config.ts");
        std::fs::write(&root_path, "export default {};").unwrap();
    ...
    #[cfg(feature = "napi")]
    #[test]
    fn test_root_goatlint_config_ts_rejects_missing_builtin_rule() {
        let root_dir = tempfile::tempdir().unwrap();
        let root_path = root_dir.path().join("goatlint.config.ts");
        std::fs::write(&root_path, "export default {};").unwrap();
        ...
        let err = ConfigStoreBuilder::from_goatlintrc(
            false,
            goatlintrc,
            None,
    ...
    #[cfg(feature = "napi")]
    #[test]
    fn test_nested_goatlint_config_ts_rejects_type_aware() {
        let root_dir = tempfile::tempdir().unwrap();
        let nested_path = root_dir.path().join("nested/goatlint.config.ts");
        std::fs::create_dir_all(nested_path.parent().unwrap()).unwrap();
    ...
    #[cfg(feature = "napi")]
    #[test]
    fn test_nested_goatlint_config_ts_rejects_type_check() {
        let root_dir = tempfile::tempdir().unwrap();
        let nested_path = root_dir.path().join("nested/goatlint.config.ts");
        std::fs::create_dir_all(nested_path.parent().unwrap()).unwrap();
        std::fs::write(&nested_path, "export default {};").unwrap();
    ...
    #[cfg(feature = "napi")]
    #[test]
    fn test_nested_goatlint_config_ts_rejects_deny_warnings() {
        let root_dir = tempfile::tempdir().unwrap();
        let nested_path = root_dir.path().join("nested/goatlint.config.ts");
        std::fs::create_dir_all(nested_path.parent().unwrap()).unwrap();
        std::fs::write(&nested_path, "export default {};").unwrap();
    ...
    #[cfg(feature = "napi")]
    #[test]
    fn test_nested_goatlint_config_ts_allows_type_aware_from_extends() {
        let root_dir = tempfile::tempdir().unwrap();
        let nested_path = root_dir.path().join("nested/goatlint.config.ts");
        std::fs::create_dir_all(nested_path.parent().unwrap()).unwrap();
        std::fs::write(&nested_path, "export default {};").unwrap();
    ...
    #[cfg(feature = "napi")]
    #[test]
    fn test_nested_goatlint_config_ts_allows_type_check_from_extends() {
        let root_dir = tempfile::tempdir().unwrap();
        let nested_path = root_dir.path().join("nested/goatlint.config.ts");
        std::fs::create_dir_all(nested_path.parent().unwrap()).unwrap();
        std::fs::write(&nested_path, "export default {};").unwrap();
    ...
    fn test_jsonc_config_discovery() {
        ...
        assert!(result.is_ok(), "Expected .goatlintrc.jsonc to be discovered and loaded");
        let config = result.unwrap();
        assert!(
            config.path.to_string_lossy().ends_with(".goatlintrc.jsonc"),
            "Expected config path to end with .goatlintrc.jsonc, got: {}",
            config.path.display()
        );
    }
    ...
    fn test_json_and_jsonc_conflict() {
        ...
        assert!(
            result.is_err(),
            "Expected an error when both .goatlintrc.json and .goatlintrc.jsonc exist"
        );
    }
    ...
    fn test_discover_configs_skips_node_modules() {
        ...
        let base_config = root_dir.path().join(".goatlintrc.json");
        ...
        std::fs::write(node_modules.join(".goatlintrc.json"), r#"{ "rules": {} }"#).unwrap();

        // Create a legitimate nested config (not in node_modules)
        let nested_dir = root_dir.path().join("packages").join("foo");
        std::fs::create_dir_all(&nested_dir).unwrap();
        std::fs::write(nested_dir.join(".goatlintrc.json"), r#"{ "rules": {} }"#).unwrap();
