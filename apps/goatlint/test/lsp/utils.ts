...
export async function lintMultiWorkspaceFixture(
  ...
  for (const fixturePath of fixturePaths) {
    snapshots.push(
      // goatlint-disable-next-line no-await-in-loop -- for snapshot consistency
...
async function getCodeActionSnapshot(
  ...
  if (diagnostics.kind !== "full") {
    throw new Error("Only full reports are supported by goatlint lsp");
  }
  ...
  for (const diagnostic of diagnostics.items) {
    ...
    for (const action of actions) {
      if (!("edit" in action)) {
        throw new Error("Only code actions with edits are supported by goatlint lsp");
      }
...
function snapshotCodeAction(
  ...
  for (const [editedUri, textEdits] of Object.entries(action.edit.changes ?? {})) {
    if (editedUri !== uri) {
      throw new Error("Only edits for the same document are supported by goatlint lsp");
    }
