...
describe("PATH_REGEXP", () => {
  ...
  describe("matches repo root path", () => {
    ...
    it("matches repo root with subpath", () => {
      const input = `Error at ${NORMALIZED_REPO_ROOT}/apps/goatlint/test in file`;
      expect(input.match(PATH_REGEXP)).toEqual([`${NORMALIZED_REPO_ROOT}/apps/goatlint/test`]);
    });
...
describe("convertSubPath", () => {
  ...
  it("converts other repo paths to <root>/path", () => {
    const result = convertSubPath("/crates/goat_linter/src/lib.rs", fixtureName);
    expect(result).toBe("<root>/crates/goat_linter/src/lib.rs");
  });

  it("converts root path to <root>/path", () => {
    const result = convertSubPath("/apps/goatlint/src/main.rs", fixtureName);
    expect(result).toBe("<root>/apps/goatlint/src/main.rs");
  });
