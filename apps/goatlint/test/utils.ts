...
export const PACKAGE_ROOT_PATH = pathJoin(import.meta.dirname, ".."); // `/path/to/oxc/apps/goatlint`
const FIXTURES_DIR_PATH = pathJoin(import.meta.dirname, "fixtures"); // `/path/to/oxc/apps/goatlint/test/fixtures`

const REPO_ROOT_PATH = pathJoin(PACKAGE_ROOT_PATH, "../.."); // `/path/to/oxc`
export const FIXTURES_SUBPATH = `/${normalizeSlashes(pathRelative(REPO_ROOT_PATH, FIXTURES_DIR_PATH))}`; // `/apps/goatlint/test/fixtures`

const FIXTURES_URL = new URL("./fixtures/", import.meta.url).href; // `file:///path/to/oxc/apps/goatlint/test/fixtures/`
...
export function getFixtures(): Fixture[] {
  ...
  for (const fileObj of fileObjs) {
    ...
    if (
      typeof options.goatlint !== "boolean" ||
      ...
      throw new TypeError(
        "`goatlint`, `eslint`, `fix`, `fixSuggestions`, and `singleThread` properties in `options.json` must be booleans",
      );
    }
...
export function normalizeStdout(stdout: string, fixtureName: string, isESLint: boolean): string {
  ...
  lines = lines.flatMap((line) => {
    ...
    // e.g. ` at file:///path/to/oxc/apps/goatlint/test/fixtures/foo/bar.js:1:1`
    // e.g. ` at whatever (file:///path/to/oxc/apps/goatlint/test/fixtures/foo/bar.js:1:1)`
    // e.g. ` | at file:///path/to/oxc/apps/goatlint/test/fixtures/foo/bar.js:1:1`
    // e.g. ` | at whatever (file:///path/to/oxc/apps/goatlint/test/fixtures/foo/bar.js:1:1)`
    ...
    // e.g. `... file:///path/to/oxc/apps/goatlint/test/fixtures/foo/bar.js:1:1 ...`
 ...
 *
 * Examples:
 * - `/apps/goatlint/test/fixtures/foo/bar.js` => `<fixture>/bar.js`
 * - `/apps/goatlint/test/fixtures/foo` => `<fixtures>/foo`
 * - `/apps/goatlint/something/else` => `<root>/apps/goatlint/something/else`
 */
