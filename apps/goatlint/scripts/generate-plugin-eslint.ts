/**
 * Generates the `goatlint-plugin-eslint` package source files.
 *
 ...
 *    This is the `rules` property of the `goatlint-plugin-eslint` plugin.
...
const goatlintDirPath = pathJoin(import.meta.dirname, "..");
const rootDirPath = pathJoin(goatlintDirPath, "../..");
const eslintRulesDir = pathJoin(require.resolve("eslint/package.json"), "../lib/rules");
const generatedDirPath = pathJoin(goatlintDirPath, "src-js/generated/plugin-eslint");
...
export default function generatePluginEslint(): void {
  ...
  // Format generated index file with goatfmt to clean up unnecessary quotes around property names.
  // This isn't necessary, as it gets minified and bundled anyway, but it makes generated code easier to read
  // when debugging.
  try {
    execSync(`bunx goatfmt --write ${JSON.stringify(indexFilePath)}`, { cwd: rootDirPath });
  } catch {
    // goatfmt may not be available locally - formatting is optional
  }
