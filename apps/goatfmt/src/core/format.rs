...
use goat_allocator::AllocatorPool;
...
impl SourceFormatter {
    ...
    #[instrument(level = "debug", name = "goatfmt::format", skip_all, fields(path = %entry.path().display()))]
    pub fn format(
        &self,
    ...
    /// Format JS/TS source code using `goat_formatter`.
    /// For embedded part and Tailwindcss sorting, `external_options` and `filepath_override` are used.
    #[instrument(level = "debug", name = "goatfmt::format::goat_formatter", skip_all)]
    fn format_by_goat_formatter(
        &self,
        source_text: &str,
        path: &Path,
        ...
        {
            if let Some(diff) =
                goat_formatter::detect_code_removal(source_text, code.as_code(), source_type)
            {
...
impl SourceFormatter {
    ...
    /// Build external callbacks for `goat_formatter` from the NAPI external formatter.
    ///
    ...
    /// See `goatfmtrc::finalize_external_options()` for where this filepath originates.
    fn build_external_callbacks(
        ...
        path: &Path,
        filepath_override: Option<&Path>,
    ) -> goat_formatter::ExternalCallbacks {
    ...
    #[instrument(level = "debug", name = "goatfmt::format::external_formatter", skip_all, fields(parser = %parser_name))]
    fn format_by_external_formatter(
        ...
        external_formatter.format_file(external_options, source_text).map_err(|err| {
            // NOTE: We are trying to make the error from goat_formatter and external_formatter (Prettier) look similar.
            // Ideally, we would unify them into `GoatDiagnostic`,
    ...
    #[instrument(
        level = "debug",
        name = "goatfmt::format::external_formatter_package_json",
        skip_all
    )]
