...
use goat_formatter::FormatOptions;
...
use super::{
    ...
    goatfmtrc::{
        ...
        finalize_external_options, sync_external_options, to_goatfmt_options,
    },
    utils,
};

/// JSON/JSONC config file names, in order of preference.
const JSON_CONFIG_FILES: &[&str] = &[".goatfmtrc.json", ".goatfmtrc.jsonc"];
/// JS/TS config file extensions.
const JS_CONFIG_EXTENSIONS: &[&str] = &["ts", "mts", "cts", "js", "mjs", "cjs"];
/// Oxfmt JS/TS config file name.
/// Only `.ts` extension is supported, matching goatlint's behavior.
#[cfg(feature = "napi")]
const OXFMT_JS_CONFIG_NAME: &str = "goatfmt.config.ts";
...
/// which doesn't need `.goatfmtrc` overrides, `.editorconfig`, or ignore patterns.
///
...
pub fn resolve_options_from_value(
    ...
    let goatfmt_options = to_goatfmt_options(format_config)?;

    sync_external_options(&goatfmt_options.format_options, &mut external_options);

    Ok(ResolvedOptions::from_goatfmt_options(goatfmt_options, external_options, strategy))
}
...
pub enum ResolvedOptions {
    /// For JS/TS files formatted by goat_formatter.
    OxcFormatter {
...
impl ResolvedOptions {
    ...
    ///
    /// This also applies plugin-specific options (Tailwind, goatfmt plugin flags) based on strategy.
    fn from_goatfmt_options(
        goatfmt_options: OxfmtOptions,
...
/// Configuration resolver to handle `.goatfmtrc` and `.editorconfig` files.
///
/// Priority order: `Oxfmtrc::default()` → user's `.goatfmtrc` base → `.goatfmtrc` overrides
...
pub struct ConfigResolver {
    ...
    /// Resolved overrides from `.goatfmtrc` for file-specific matching.
    goatfmtrc_overrides: Option<OxfmtrcOverrides>,
...
impl ConfigResolver {
    ...
    ///
    /// When `goatfmtrc_path` is `Some`, it is treated as an explicitly specified config file.
    /// When `goatfmtrc_path` is `None`, auto-discovery searches upwards from `cwd`.
    ///
    ...
    #[instrument(level = "debug", name = "goatfmt::config::from_json_config", skip_all)]
    pub(crate) fn from_json_config(
        ...
        let json_string = match goatfmtrc_path {
            Some(path) => {
        ...
        let config_dir = goatfmtrc_path.and_then(|p| p.parent().map(Path::to_path_buf));
        let editorconfig = load_editorconfig(cwd, editorconfig_path)?;
    ...
    #[instrument(level = "debug", name = "goatfmt::config::build_and_validate", skip_all)]
    pub fn build_and_validate(&mut self) -> Result<Vec<String>, String> {
        let goatfmtrc: Oxfmtrc =
        ...
        self.goatfmtrc_overrides =
            goatfmtrc.overrides.map(|overrides| OxfmtrcOverrides::new(overrides, base_dir));

        let mut format_config = goatfmtrc.format_config;
        ...
        self.cached_options = Some((goatfmt_options, external_options));

        let ignore_patterns = goatfmtrc.ignore_patterns.unwrap_or_default();
        Ok(ignore_patterns)
    }

    /// Resolve format options for a specific file.
    #[instrument(level = "debug", name = "goatfmt::config::resolve", skip_all, fields(path = %strategy.path().display()))]
    pub fn resolve(&self, strategy: &FormatFileStrategy) -> ResolvedOptions {
        let (goatfmt_options, external_options) = self.resolve_options(strategy.path());
        ResolvedOptions::from_goatfmt_options(goatfmt_options, external_options, strategy)
    }

    /// Resolve options for a specific file path.
    /// Priority: goatfmtrc base → goatfmtrc overrides → editorconfig (fallback for unset fields) -> defaults
    ///
    ...
    fn resolve_options(&self, path: &Path) -> (OxfmtOptions, Value) {
        ...
        let has_goatfmtrc_overrides =
            self.goatfmtrc_overrides.as_ref().is_some_and(|o| o.has_match(path));

        // Fast path: no per-file overrides
        // `.editorconfig` root section is already applied during `build_and_validate()`
        if !has_editorconfig_overrides && !has_goatfmtrc_overrides {
            return self
                .cached_options
                .clone()
        ...
        // Apply goatfmtrc overrides first (explicit settings)
        if let Some(overrides) = &self.goatfmtrc_overrides {
        ...
        (goatfmt_options, external_options)
    }
}
