...
use goat_formatter::get_supported_source_type;
...
impl FormatFileStrategy {
    ...
    /// Returns `true` if this strategy supports the `prettier-plugin-goatfmt` (js-in-xxx).
    #[cfg(feature = "napi")]
    pub fn needs_goatfmt_plugin(&self) -> bool {
        matches!(
            self,
...
/// Parsers(files) that can embed JS/TS code and benefit from goatfmt plugin.
