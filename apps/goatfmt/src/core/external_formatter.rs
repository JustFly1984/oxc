...
impl ExternalFormatter {
    ...
    pub fn init(&self, num_threads: usize) -> Result<Vec<String>, String> {
        debug_span!("goatfmt::external::init", num_threads = num_threads)
    ...
    /// Convert this external formatter to the goat_formatter::ExternalCallbacks type.
    ...
    pub fn to_external_callbacks(
        ...
        let embedded_callback: Option<EmbeddedFormatterCallback> = if needs_embedded {
            ...
            Some(Arc::new(move |language: &str, code: &str| {
                ...
                debug_span!("goatfmt::external::format_embedded", parser = parser_name).in_scope(
                    || {
        ...
        let embedded_doc_callback: Option<EmbeddedDocFormatterCallback> = if needs_embedded {
            ...
            Some(Arc::new(move |allocator, group_id_builder, language: &str, texts: &[&str]| {
                ...
                debug_span!("goatfmt::external::format_embedded_doc", parser = parser_name)
                    .in_scope(|| {
        ...
        let tailwind_callback: Option<TailwindCallback> = if needs_tailwind {
            ...
            Some(Arc::new(move |classes: Vec<String>| {
                debug_span!("goatfmt::external::sort_tailwind", classes_count = classes.len())
...
/// but `goat_formatter` treats them as generic language names.
