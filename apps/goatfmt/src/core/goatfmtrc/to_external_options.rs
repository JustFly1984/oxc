...
use goat_formatter::{FormatOptions, IndentStyle, LineEnding};

use crate::core::FormatFileStrategy;

/// Syncs resolved `FormatOptions` values into the raw config JSON.
/// This ensures `external_formatter`(Prettier) receives the same options that `goat_formatter` uses.
///
/// Only options that meet one of these criteria need to be mapped:
/// - 1. Different defaults between Prettier and goat_formatter
...
/// - `_goatfmtPluginOptionsJson`: Bundled options for `prettier-plugin-goatfmt`
///
...
pub fn finalize_external_options(config: &mut Value, strategy: &FormatFileStrategy) {
    ...
    // Build goatfmt plugin options JSON for js-in-xxx parsers
    #[cfg(feature = "napi")]
    if let FormatFileStrategy::ExternalFormatter { path, .. } = strategy
        && strategy.needs_goatfmt_plugin()
    {
        let mut goatfmt_plugin_options = serde_json::Map::new();
        for key in [
            "printWidth",
            "useTabs",
            "tabWidth",
            ...
            if let Some(value) = obj.get(key) {
                goatfmt_plugin_options.insert(key.to_string(), value.clone());
            }
        }
        ...
        goatfmt_plugin_options
            .insert("filepath".to_string(), Value::String(path.to_string_lossy().to_string()));

        if let Ok(json_str) = serde_json::to_string(&Value::Object(goatfmt_plugin_options)) {
            obj.insert("_goatfmtPluginOptionsJson".to_string(), Value::String(json_str));
        }
    }
...
mod tests {
    use super::*;
    use crate::core::goatfmtrc::{FormatConfig, Oxfmtrc, to_goatfmt_options};
    ...
    fn test_sync_external_options_defaults() {
        ...
        sync_external_options(&goatfmt_options.format_options, &mut raw_config);
    ...
    fn test_sync_external_options_with_user_values() {
        ...
        // goatfmt extensions are preserved (for caching)
    ...
    fn test_sync_external_options_preserves_overrides() {
        ...
        let mut raw_config: Value = serde_json::from_str(json_string).unwrap();
        let goatfmtrc: Oxfmtrc = serde_json::from_str(json_string).unwrap();
        let goatfmt_options = to_goatfmt_options(goatfmtrc.format_config).unwrap();
    ...
    #[test]
    fn test_finalize_external_options_removes_goatfmt_extensions() {
        ...
        let obj = raw_config.as_object().unwrap();
        // goatfmt extensions are removed by finalize_external_options
        assert!(!obj.contains_key("overrides"));
        assert!(!obj.contains_key("ignorePatterns"));
    ...
    #[test]
    #[cfg(feature = "napi")]
    fn test_finalize_external_options_sets_goatfmt_plugin_filepath() {
        use std::path::PathBuf;
        ...
        let plugin_options_json = obj
            .get("_goatfmtPluginOptionsJson")
            .and_then(Value::as_str)
            .expect("Expected `_goatfmtPluginOptionsJson` to be set");
