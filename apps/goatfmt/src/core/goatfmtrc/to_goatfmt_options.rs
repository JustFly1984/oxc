...
///
/// Contains `FormatOptions` for `goat_formatter` plus additional Oxfmt-specific options.
...
pub fn to_goatfmt_options(config: FormatConfig) -> Result<OxfmtOptions, String> {
    ...
    if let Some(jsdoc_config) = config.jsdoc.and_then(JsdocUserConfig::into_config) {
        let mut opts = goat_formatter::JsdocOptions::default();
        ...
        if let Some(ref v) = jsdoc_config.line_wrapping_style {
            opts.line_wrapping_style = match v.as_str() {
                "greedy" => goat_formatter::LineWrappingStyle::Greedy,
        ...
        if let Some(ref v) = jsdoc_config.comment_line_strategy {
            opts.comment_line_strategy = match v.as_str() {
                "singleLine" => goat_formatter::CommentLineStrategy::SingleLine,
...
fn build_toml_options(format_options: &FormatOptions) -> TomlFormatterOptions {
    TomlFormatterOptions {
        ...
        // NOTE: Need to align with `goat_formatter` and Prettier defaults,
...
#[cfg(test)]
mod tests {
    use goat_formatter::{Expand, GroupName};
    ...
    fn test_config_parsing() {
        ...
        let goatfmt_options = to_goatfmt_options(config).unwrap();

        assert!(goatfmt_options.format_options.indent_style.is_tab());
        assert_eq!(goatfmt_options.format_options.indent_width.value(), 4);
        assert_eq!(goatfmt_options.format_options.line_width.value(), 100);
        assert!(!goatfmt_options.format_options.quote_style.is_double());
        assert!(goatfmt_options.format_options.semicolons.is_as_needed());

        let sort_imports = goatfmt_options.format_options.sort_imports.unwrap();
    ...
    fn test_ignore_unknown_fields() {
        ...
        // Should use defaults
        assert!(goatfmt_options.format_options.indent_style.is_space());
        assert_eq!(goatfmt_options.format_options.indent_width.value(), 2);
        assert_eq!(goatfmt_options.format_options.line_width.value(), 100);
        assert_eq!(goatfmt_options.format_options.sort_imports, None);
    }
    ...
    fn test_arrow_parens_normalization() {
        ...
        let goatfmt_options = to_goatfmt_options(config).unwrap();
        assert!(goatfmt_options.format_options.arrow_parentheses.is_as_needed());

        // Test "always" remains unchanged
        let config: FormatConfig = serde_json::from_str(r#"{"arrowParens": "always"}"#).unwrap();
        let goatfmt_options = to_goatfmt_options(config).unwrap();
        assert!(goatfmt_options.format_options.arrow_parentheses.is_always());
    }
    ...
    fn test_object_wrap_normalization() {
        ...
        let goatfmt_options = to_goatfmt_options(config).unwrap();
        assert_eq!(goatfmt_options.format_options.expand, Expand::Auto);

        // Test "collapse" -> "never" normalization
        let config: FormatConfig = serde_json::from_str(r#"{"objectWrap": "collapse"}"#).unwrap();
        let goatfmt_options = to_goatfmt_options(config).unwrap();
        assert_eq!(goatfmt_options.format_options.expand, Expand::Never);
    }
    ...
    fn test_sort_imports_config() {
        ...
        .unwrap();
        assert!(to_goatfmt_options(config).is_ok());
        let config: FormatConfig = serde_json::from_str(
            r#"{
                "experimentalSortImports": {
                    "partitionByNewline": true,
        ...
        .unwrap();
        assert!(to_goatfmt_options(config).is_err_and(|e| e.contains("newlinesBetween")));
        ...
        .unwrap();
        assert!(to_goatfmt_options(config).is_err_and(|e| e.contains("start")));
        ...
        .unwrap();
        assert!(to_goatfmt_options(config).is_err_and(|e| e.contains("end")));
        ...
        .unwrap();
        assert!(to_goatfmt_options(config).is_err_and(|e| e.contains("consecutive")));
        ...
        .unwrap();
        assert!(to_goatfmt_options(config).is_err_and(|e| e.contains("partitionByNewline")));
    }

    #[test]
    fn test_bool_for_object_options() {
        let config: FormatConfig = serde_json::from_str(r#"{"sortImports": true}"#).unwrap();
        assert!(to_goatfmt_options(config).unwrap().format_options.sort_imports.is_some());

        let config: FormatConfig = serde_json::from_str(r#"{"sortImports": false}"#).unwrap();
        assert!(to_goatfmt_options(config).unwrap().format_options.sort_imports.is_none());

        let config: FormatConfig = serde_json::from_str(r#"{"sortTailwindcss": true}"#).unwrap();
        assert!(to_goatfmt_options(config).unwrap().format_options.sort_tailwindcss.is_some());

        let config: FormatConfig = serde_json::from_str(r#"{"sortTailwindcss": false}"#).unwrap();
        assert!(to_goatfmt_options(config).unwrap().format_options.sort_tailwindcss.is_none());

        let config: FormatConfig = serde_json::from_str(r#"{"jsdoc": true}"#).unwrap();
        assert!(to_goatfmt_options(config).unwrap().format_options.jsdoc.is_some());

        let config: FormatConfig = serde_json::from_str(r#"{"jsdoc": false}"#).unwrap();
        assert!(to_goatfmt_options(config).unwrap().format_options.jsdoc.is_none());
    }
}
