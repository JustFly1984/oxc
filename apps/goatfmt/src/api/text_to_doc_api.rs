...
pub fn run(
    source_ext: &str,
    source_text: &str,
    goatfmt_plugin_options_json: &str,
    parent_context: &str,
    ...
    let doc_json = if let Some(kind) = fragment_kind {
        run_fragment(source_ext, source_text, goatfmt_plugin_options_json, kind)?
    } else {
        run_full(
            source_ext,
            source_text,
            goatfmt_plugin_options_json,
...
#[instrument(level = "debug", name = "goatfmt::text_to_doc::full", skip_all, fields(%source_ext))]
fn run_full(
    source_ext: &str,
    source_text: &str,
    goatfmt_plugin_options_json: &str,
    ...
    // Options and paths in `_goatfmtPluginOptionsJson` are already resolved to absolute paths
    ...
    let (options, parent_filepath) = parse_options_and_filepath(goatfmt_plugin_options_json);
    ...
    let resolved_options = resolve_options_from_value(options, &strategy, None)
        .expect("`_goatfmtPluginOptionsJson` should contain valid config");
...
#[instrument(level = "debug", name = "goatfmt::text_to_doc::fragment", skip_all, fields(%source_ext, ?kind))]
fn run_fragment(
    source_ext: &str,
    source_text: &str,
    goatfmt_plugin_options_json: &str,
    ...
    let (options, _parent_filepath) = parse_options_and_filepath(goatfmt_plugin_options_json);
    ...
    let formatter = Formatter::new(
        ...
        FormatOptions {
            ...
            quote_style: goat_formatter::QuoteStyle::Single,
            ..*format_options
        },
    );
...
///
/// The `filepath` field (set by `core::goatfmtrc::finalize_external_options`) stores
/// the parent file path (e.g. `App.vue`) for js-in-xxx formatting.
fn parse_options_and_filepath(goatfmt_plugin_options_json: &str) -> (Value, PathBuf) {
    let Ok(Value::Object(mut obj)) = serde_json::from_str(goatfmt_plugin_options_json) else {
        unreachable!("`_goatfmtPluginOptionsJson` should be a valid JSON object");
    };
    let Some(Value::String(s)) = obj.remove("filepath") else {
        unreachable!("Expected `filepath` in `_goatfmtPluginOptionsJson`");
    };
