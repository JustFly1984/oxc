...
/// Converts `goat_formatter` IR (`FormatElement` slice) into Prettier Doc.
///
...
/// Simulates a subset of the `goat_formatter` printer's runtime state.
///
...
/// `crates/goat_formatter/src/formatter/printer/mod.rs`.
///
...
fn build_doc(start_info: Option<&StartTagInfo>, children: Vec<Value>) -> Value {
    ...
    match info {
        ...
        // goat_formatter's `Tag::StartIndentIfGroupBreaks` doesn't have this option yet.
