...
pub enum Mode {
    ...
    #[cfg(feature = "napi")]
    Lsp,
    /// Initialize `.goatfmtrc.json` with default values
    // NOTE: Actual logic is handled by JS side.
    #[cfg(feature = "napi")]
    Init,
    /// Migrate Prettier configuration to `.goatfmtrc.json`
    // NOTE: Actual logic is handled by JS side.
    #[cfg(feature = "napi")]
...
fn mode() -> impl bpaf::Parser<Mode> {
    ...
    #[cfg(feature = "napi")]
    {
        let init = bpaf::long("init")
            .help("Initialize `.goatfmtrc.json` with default values")
            .req_flag(Mode::Init)
            .hide_usage();
        let migrate = bpaf::long("migrate")
            .help("Migrate configuration to `.goatfmtrc.json` from specified source\nAvailable sources: prettier, biome")
