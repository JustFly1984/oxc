...
use goat_diagnostics::DiagnosticService;
...
impl CliRunner {
    ...
    pub fn run(self) -> CliRunResult {
        ...
        let print_stats = |stdout, stderr| {
            ...
            if no_config {
                #[cfg(feature = "napi")]
                let hint = "No config found, using defaults. Please add a config file or try `goatfmt --init` if needed.\n";
                #[cfg(not(feature = "napi"))]
                let hint =
                    "No config found, using defaults. Please add `.goatfmtrc.json` if needed.\n";
