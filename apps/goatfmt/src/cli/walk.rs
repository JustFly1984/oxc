...
impl Walk {
    pub fn build(
        ...
        for path in paths {
            ...
            // Instead of `goatlint`'s `--ignore-pattern=PAT`,
            // `goatfmt` supports `!` prefix in paths like Prettier.
        ...
        // 2. Handle `goatfmtrc.ignorePatterns`
        ...
        match entry {
            Ok(entry) => {
                ...
                if file_type.is_file() {
                    ...
                    // Tier 1 = `.js`, `.tsx`, etc: JS/TS files supported by `goat_formatter`
                    // Tier 2 = `.toml`, etc: Some files supported by `goatfmt` directly
