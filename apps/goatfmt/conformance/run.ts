// goatlint-disable no-console, no-await-in-loop
  ...
  },
  {
    ...
    notes: {
      "prettier/js/multiparser-html/issue-10691.js":
        "js-in-html(`<script>`)-in-js needs lot more work; Please see goat_formatter/src/print/template/embed/html.rs",
...
async function runCategory(category: Category, fixtures: Fixture[]): Promise<CategoryResult> {
  ...
  for (const options of category.optionSets) {
    ...
    for (const fixture of fixtures) {
      ...
      const [goatfmtResult, prettierResult] = await compareWithPrettier(
        fixture.name,
        content,
        options,
      );

      if (goatfmtResult === prettierResult) {
        passed++;
      } else {
        failures.push({
          name: fixture.name,
          note: category.notes?.[fixture.name],
          goatfmt: goatfmtResult,
...
async function compareWithPrettier(
  ...
  let goatfmtResult: string;
  const res = await format(fileName, content, options);
  if (res.errors.length !== 0) {
    goatfmtResult = "ERROR";
  } else {
    goatfmtResult = res.code;
  }

  return [goatfmtResult, prettierResult];
}
...
function writeDiffFile(
  ...
  for (const entry of entries) {
    ...
    lines.push("`````");
    lines.push("");
    lines.push("### Actual (goatfmt)");
    lines.push("");
    lines.push(`\`\`\`\`\`${lang}`);
    lines.push(entry.failure.goatfmt);
    lines.push("`````");
    lines.push("");
