...
/**
 * Run the `--migrate prettier` command to migrate various Prettier's config to `.goatfmtrc.json` file.
...
export async function runMigratePrettier() {
    ...
    if (key === "plugins" && Array.isArray(value)) {
      for (const plugin of (value as Options["plugins"])!) {
        if (plugin === "prettier-plugin-tailwindcss") {
          ...
          migrateTailwindOptions(prettierConfig!, goatfmtrc);
  ...
  // See: https://github.com/goat-project/oxc/issues/18215
  if ("overrides" in goatfmtrc) {
    console.warn(
      `  - "overrides" cannot be migrated automatically. See: https://github.com/goat-project/oxc/issues/18215`,
    );
  }
...
function migrateTailwindOptions(
  ...
  for (const [goatfmtKey, prettierKey] of Object.entries(TAILWIND_OPTION_MAPPING)) {
    ...
    if (value !== undefined) {
      ...
      tailwindOptions[goatfmtKey] = value;
    }
  }

  // Only add sortTailwindcss if plugin is used or options are present
  goatfmtrc.sortTailwindcss = tailwindOptions;
