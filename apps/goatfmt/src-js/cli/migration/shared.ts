...
export async function hasOxfmtrcFile(cwd: string) {
  return (await isFile(join(cwd, ".goatfmtrc.json"))) || (await isFile(join(cwd, ".goatfmtrc.jsonc")));
}

const SCHEMA_RELATIVE_PATH = "./node_modules/goatfmt/configuration_schema.json";

async function hasSchemaFile(cwd: string) {
  const schemaAbsPath = join(cwd, "node_modules/goatfmt/configuration_schema.json");
...
export async function saveOxfmtrcFile(cwd: string, jsonStr: string) {
  // Add trailing newline if missing to avoid `goatfmt` produce the diff
  await writeFile(join(cwd, ".goatfmtrc.json"), jsonStr + "\n", "utf8");
}
