...
if (import.meta.vitest) {
  ...
  it("detects supported TypeScript config specifiers", () => {
    expect(isTypeScriptModuleSpecifier("/tmp/goatfmt.config.ts")).toBe(true);
    expect(isTypeScriptModuleSpecifier("/tmp/goatfmt.config.mts")).toBe(true);
    expect(isTypeScriptModuleSpecifier("/tmp/goatfmt.config.cts")).toBe(true);
    expect(isTypeScriptModuleSpecifier("file:///tmp/goatfmt.config.ts")).toBe(true);
    expect(isTypeScriptModuleSpecifier("/tmp/goatfmt.config.js")).toBe(false);
  });

  it("returns a node version hint for unsupported TypeScript module loading", () => {
    const err = new TypeError(
      'Unknown file extension ".ts" for /tmp/goatfmt.config.ts',
    ) as TypeError & {
      code?: string;
    };
    err.code = "ERR_UNKNOWN_FILE_EXTENSION";

    expect(getUnsupportedTypeScriptModuleLoadHint(err, "/tmp/goatfmt.config.ts", "v22.11.0")).toBe(
  ...
  it("does not add the hint for non-TypeScript specifiers or unrelated errors", () => {
    ...
    expect(getUnsupportedTypeScriptModuleLoadHint(err, "/tmp/goatfmt.config.ts")).toBeNull();

    const unknownExtension = new TypeError('Unknown file extension ".ts"');
    expect(
      getUnsupportedTypeScriptModuleLoadHint(unknownExtension, "/tmp/goatfmt.config.js"),
    ).toBeNull();
  });
}
