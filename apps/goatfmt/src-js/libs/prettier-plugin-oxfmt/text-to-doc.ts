...
export const textToDoc: Parser<Doc>["parse"] = async (embeddedSourceText, textToDocOptions) => {
  // `_goatfmtPluginOptionsJson` is a JSON string bundled by Rust (`goatfmtrc::finalize_external_options`),
  // containing format options + parent filepath for the Rust-side `goat_formatter`.
  const { parser, parentParser, filepath, _goatfmtPluginOptionsJson } = textToDocOptions;

  // For (j|t)s-in-xxx, default `parser` is either `babel`, `babel-ts` or `typescript`
  // We need to infer `SourceType::from_extension(ext)` for `goat_formatter`.
  ...
  const doc = await jsTextToDoc(
    embeddedSourceExt,
    embeddedSourceText,
    _goatfmtPluginOptionsJson as string,
    parentContext,
  );

  if (doc === null) {
    throw new Error("`goatfmt::textToDoc()` failed. Use `OXC_LOG` env var to see Rust-side logs.");
  }
