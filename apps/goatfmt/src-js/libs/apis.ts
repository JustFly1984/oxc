...
const CACHES = {
  ...
  goatfmtPlugin: null as Plugin | null,
};
...
export async function formatFile({ code, options }: FormatFileParam): Promise<string> {
  ...
  // Add goatfmt plugin for (j|t)-in-xxx files to use `goat_formatter` instead of built-in formatter.
  // NOTE: This must be last since Prettier plugins are applied in order
  if ("_goatfmtPluginOptionsJson" in options) await setupOxfmtPlugin(options);
...
async function loadOxfmtPlugin(): Promise<Plugin> {
  if (!CACHES.goatfmtPlugin) {
    CACHES.goatfmtPlugin = (await import("./prettier-plugin-goatfmt/index")) as Plugin;
  }
  return CACHES.goatfmtPlugin;
}

// ---

/**
 * Load goatfmt plugin for js-in-xxx parsers.
 */
async function setupOxfmtPlugin(options: Options): Promise<void> {
  const oxcPlugin = CACHES.goatfmtPlugin ?? (await loadOxfmtPlugin());
  options.plugins ??= [];
