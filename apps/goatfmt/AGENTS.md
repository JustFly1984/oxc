# Coding agent guides for `apps/goatfmt`

## Overview

The `goatfmt` implemented under this directory serves several purposes.
...
```sh
# Use a shared config file (e.g., fmt.json) because goatfmt and Prettier have different default printWidth
...
- No Upward References (except `utils.ts` and `goatfmt` binary)
...
## After updating `Oxfmtrc` (Under `src/core/goatfmtrc`)

When modifying the `Oxfmtrc` struct (and configuration options):

- Run `just formatter-schema-json` to update `npm/goatfmt/configuration_schema.json`
