...
describe("--migrate biome", () => {
  it("should create .goatfmtrc.json when no biome config exists", async () => {
    const tempDir = await fs.mkdtemp(join(tmpdir(), "goatfmt-migrate-biome-test"));
  ...
  it("should migrate biome.json config to .goatfmtrc.json", async () => {
    const tempDir = await fs.mkdtemp(join(tmpdir(), "goatfmt-migrate-biome-test"));
    ...
    try {
      ...
      expect(goatfmtrc.useTabs).toBe(false);
  ...
  it("should migrate biome.jsonc config with comments", async () => {
    ...
    try {
      ...
      expect(goatfmtrc.printWidth).toBe(100);
      expect(goatfmtrc.useTabs).toBe(true);
    } finally {
      await fs.rm(tempDir, { recursive: true, force: true });
    }
  });
  ...
  it("should use Biome defaults when options are not specified", async () => {
    ...
    try {
      ...
      expect(goatfmtrc.printWidth).toBe(80);
      expect(goatfmtrc.useTabs).toBe(true);
      expect(goatfmtrc.tabWidth).toBe(2);
      expect(goatfmtrc.singleQuote).toBe(false);
      expect(goatfmtrc.jsxSingleQuote).toBe(false);
      expect(goatfmtrc.quoteProps).toBe("as-needed");
      expect(goatfmtrc.trailingComma).toBe("all");
      expect(goatfmtrc.semi).toBe(true);
      expect(goatfmtrc.arrowParens).toBe("always");
      expect(goatfmtrc.bracketSameLine).toBe(false);
      expect(goatfmtrc.bracketSpacing).toBe(true);
    } finally {
      await fs.rm(tempDir, { recursive: true, force: true });
    }
  });
  ...
  it("should migrate arrowParentheses correctly", async () => {
    ...
    try {
      ...
      expect(goatfmtrc.arrowParens).toBe("avoid");
    } finally {
      await fs.rm(tempDir, { recursive: true, force: true });
    }
  });
  ...
  it("should migrate attributePosition to singleAttributePerLine", async () => {
    ...
    try {
      ...
      expect(goatfmtrc.singleAttributePerLine).toBe(true);
    } finally {
      await fs.rm(tempDir, { recursive: true, force: true });
    }
  });
  ...
  it("should migrate ignore patterns from files.includes", async () => {
    ...
    try {
      ...
      expect(Object.keys(goatfmtrc).at(-1)).toBe("ignorePatterns");
    } finally {
      await fs.rm(tempDir, { recursive: true, force: true });
    }
  });
  ...
  it("should not include force-ignore patterns (starting with !!) in ignorePatterns", async () => {
    ...
    try {
      ...
      expect(goatfmtrc.ignorePatterns).toEqual(["dist"]);
    } finally {
      await fs.rm(tempDir, { recursive: true, force: true });
    }
  });
