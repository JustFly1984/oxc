...
describe("vite_config", () => {
  ...
  it("skip: parent config is found when vite.config.ts without fmt is skipped", async () => {
    ...
    // parent has .goatfmtrc.json with semi: false
  ...
  it("priority: goatfmt.config.ts takes precedence over vite.config.ts", async () => {
    // `goatfmt.config.ts` has `semi: false`, `vite.config.ts` has `semi: true`
    // goatfmt.config.ts should win, so `const a = 1;` (with semicolon) should be flagged
