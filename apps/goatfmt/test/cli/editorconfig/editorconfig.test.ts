...
describe("editorconfig", () => {
  ...
  //
  // Expected: useTabs=false, tabWidth=2 (goatfmtrc wins)
  // - Indentation should use 2 spaces, NOT tabs
  it(".goatfmtrc overrides .editorconfig", async () => {
    const cwd = join(fixturesDir, "with_goatfmtrc");
    const snapshot = await runWriteModeAndSnapshot(cwd, ["test.js"]);
    expect(snapshot).toMatchSnapshot();
  });
  ...
  //   tabWidth=8
  //
  // Expected: useTabs=true (from editorconfig), tabWidth=8 (from goatfmtrc)
