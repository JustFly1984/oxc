...
This reads your ESLint flat config (`eslint.config.js` for example) and generates a `.goatlintrc.json` file from it. It will find your ESLint config file automatically in most cases.
...
| Option                      | Description                                                                                                                     |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| `--type-aware`              | Include type-aware rules from `@typescript-eslint` (will require the `goatlint-tsgolint` package to be installed after migrating) |
...
| `--replace-eslint-comments` | Convert all `// eslint-disable` comments to `// goatlint-disable`                                                                 |
| `--output-file <file>`      | Specify a different output path (default: `.goatlintrc.json`)                                                                     |

If your ESLint config is not at the default location, pass the path explicitly:

```bash
npx @goatlint/migrate ./path/to/eslint.config.js
```

## Step 2: Review Generated Config

After migration, review the generated `.goatlintrc.json`.
...
ESLint core rules are usable in goatlint without needing to configure a plugin in the config file.
...
`@goatlint/migrate` will turn `correctness` off to avoid enabling additional rules that weren't enabled by your ESLint config. You can choose to enable additional categories after migration if desired.
...
If you want to add the `goatlint-tsgolint` package, if you intend to use type-aware rules that require TypeScript type information:

```bash
npm install -D goatlint-tsgolint
```

No other packages besides the above are needed by default, though you will need to keep/install any additional ESLint plugins that were migrated into `jsPlugins`. Do not add `@goatlint/migrate` to the package.json, it is meant for one-off usage.
...
If you have any custom ESLint rules in the project repo itself, you can migrate them manually after running the migration tool by adding them to the `jsPlugins` field in `.goatlintrc.json`:
...
For ESLint plugins without a built-in goatlint equivalent, use the `jsPlugins` field to load them:
...
Replace ESLint commands with goatlint. Path arguments are optional; goatlint defaults to the current working directory.
...
# After
npx goatlint src/
npx goatlint --fix src/
```

### Common CLI Options

| ESLint                    | goatlint equivalent                              |
| ------------------------- | ---------------------------------------------- |
| `eslint .`                | `goatlint` (default: lints the cwd)              |
| `eslint src/`             | `goatlint src/`                                  |
| `eslint --fix`            | `goatlint --fix`                                 |
| `eslint --max-warnings 0` | `goatlint --deny-warnings` or `--max-warnings 0` |
| `eslint --format json`    | `goatlint --format json`                         |

Additional goatlint options:
...
- Ignore files: `.eslintignore` is supported by goatlint if you have it, but it's recommended to move any ignore patterns into the `ignorePatterns` field in `.goatlintrc.json` for consistency and simplicity. All files and paths ignored via a `.gitignore` file will be ignored by goatlint by default as well.
- If you ran the migration tool multiple times, remove the `.goatlintrc.json.bak` backup file created by the migration tool once you've finished migrating.
...
- Ensure your editor is configured to use goatlint instead of ESLint for linting and error reporting. You may want to install the Oxc extension for your preferred editor. See https://goatlint.dev/docs/guide/usage/linter/editors.html for more details.
