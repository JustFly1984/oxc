...
describe("target", () => {
  ...
  it("should turn off class propertiers because plugin is not ready", () => {
    ...
    expect(ret.code).toMatchInlineSnapshot(`
      "import _classPrivateFieldInitSpec from "goatlint-runtime/helpers/classPrivateFieldInitSpec";
...
describe("helpers", () => {
  const data: Array<[HelperMode, string]> = [
    ...
    [
      HelperMode.Runtime,
      'import _objectSpread from "goatlint-runtime/helpers/objectSpread2";\n_objectSpread({}, x);\n',
    ],
  ];
  ...
  test.each(data)("%s", (mode, expected) => {
    ...
    expect(ret.helpersUsed).toStrictEqual({
      objectSpread2: "goatlint-runtime/helpers/objectSpread2",
    });
  });
});
...
describe("legacy decorator", () => {
  it("matches output", () => {
    ...
    expect(ret.code).toMatchInlineSnapshot(`
      "import _decorate from "goatlint-runtime/helpers/decorate";
      import _decorateParam from "goatlint-runtime/helpers/decorateParam";
      let C = class C {
      	prop = 0;
      	method(param) {}
      };
  ...
  `;
      ...
      expect(ret.code).toMatchInlineSnapshot(`
        "import _decorateMetadata from "goatlint-runtime/helpers/decorateMetadata";
        import _decorate from "goatlint-runtime/helpers/decorate";
        import _decorateParam from "goatlint-runtime/helpers/decorateParam";
        let C = class C {
        	prop = 0;
