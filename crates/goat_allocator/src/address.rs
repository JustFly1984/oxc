...
impl Address {
    ...
    ///
    /// ```ignore
    /// use goat_allocator::{Address, GetAddress, Vec};
    /// use goat_span::SPAN;
    ///
...
pub trait UnstableAddress {
    ...
    /// * `goat_ast::AstKind` - guaranteed stable address while `Semantic` is alive.
    /// * `goat_traverse::Ancestor` - guaranteed stable address while traversing descendents of the ancestor.
    ///
