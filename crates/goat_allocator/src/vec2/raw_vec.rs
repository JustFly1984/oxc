...
impl<'a, T, A: Alloc> RawVec<'a, T, A> {
    ...
    */
    ...
    // mark as `#[inline]` because this function is common case in the goat_parser.
    #[inline]
...
impl<T, A: Alloc> RawVec<'_, T, A> {
    ...
    fn grow_amortized(&mut self, len: u32, additional: usize) -> Result<(), AllocError> {
        unsafe {
            ...
            // in the goat_transformer, goat_minifier and goat_mangler, but would only get a
            // tiny performance improvement in the goat_parser.
            //
            // The reason is that only the goat_parser has a lot of tiny `Vec`s without
