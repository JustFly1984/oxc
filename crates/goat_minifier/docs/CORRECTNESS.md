...
All JavaScript coercion rules are preserved using `goat_ecmascript` conversions:
...
Located in `crates/goat_minifier/tests/`:

```bash
cargo test -p goat_minifier
```
...
```bash
cargo run -p goat_minifier --example minifier test.js --twice
```
