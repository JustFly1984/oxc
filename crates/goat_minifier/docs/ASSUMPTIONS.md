...
These assumptions are validated using ECMAScript operations from [`goat_ecmascript`](../goat_ecmascript), which implements spec-compliant behavior for type conversions, side effect analysis, and constant evaluation.
...
`.toString()`, `.valueOf()`, and `[Symbol.toPrimitive]()` have no side effects. The minifier uses `goat_ecmascript` type conversion utilities that assume standard coercion behavior.
