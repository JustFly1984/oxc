use goat_data_structures::code_buffer::CodeBuffer;
...
///
/// This is a workaround to avoid circular dependency. `goat_span` crate depends on `goat_estree` crate,
/// so we can't import `Span` directly from `goat_span` here.
