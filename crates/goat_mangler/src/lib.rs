...
use goat_allocator::{Allocator, BitSet, HashSet, Vec};
use goat_ast::AstKind;
use goat_ast::ast::{Declaration, Program, Statement};
use goat_data_structures::inline_string::InlineString;
use goat_semantic::{AstNodes, Reference, Scoping, Semantic, SemanticBuilder, SymbolId};
use goat_span::{CompactStr, Ident, SourceType, Str};
...
pub struct MangleOptions {
    ...
    ///
    /// [`ModuleKind::Module`]: goat_span::ModuleKind::Module
    /// [`ModuleKind::CommonJS`]: goat_span::ModuleKind::CommonJS
...
impl<'t> Mangler<'t> {
    ...
    /// Pass the symbol table to goat_codegen to generate the mangled code.
    #[must_use]
...
fn generate_reserved_names<'a, const CAPACITY: usize>(
    ...
    for _ in 0..names_needed {
        let name = loop {
            ...
            let n = name.as_str();
            if !goat_syntax::keyword::is_reserved_keyword(n)
                && !is_special_name(n)
