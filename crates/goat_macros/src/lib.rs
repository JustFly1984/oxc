...
/// for [our website](https://goatlint.dev). Please make sure they are clear and
...
/// documentation](https://goatlint.dev/docs/contribute/linter.html#rule-category) for
...
/// allows the `goatlint` CLI to surface this rule to the external `tsgolint`
...
/// ```
#[proc_macro]
pub fn declare_goat_lint(input: TokenStream) -> TokenStream {
    let metadata = parse_macro_input!(input as declare_goat_lint::LintRuleMeta);
    declare_goat_lint::declare_goat_lint(metadata)
}

/// Same as `declare_goat_lint`, but doesn't do imports.
/// Enables multiple usages in a single file.
#[proc_macro]
pub fn declare_goat_lint_test(input: TokenStream) -> TokenStream {
    let mut metadata = parse_macro_input!(input as declare_goat_lint::LintRuleMeta);
