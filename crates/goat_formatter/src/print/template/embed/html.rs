...
use goat_allocator::StringBuilder;
use goat_ast::ast::*;
use goat_span::GetSpan;
use goat_syntax::line_terminator::LineTerminatorSplitter;
...
pub(super) fn format_html_doc<'a>(
    ...
    let Some(Ok(EmbeddedDocResult::DocWithPlaceholders {
        ...
        // But currently, `goatfmt/prettier_compat/from_prettier_doc.rs` does not support this.
        // So `format_embedded_doc()` will return `Err`.
        //
        // In Prettier, `conditionalGroup` is only used by JS and YAML formatting.
        // And we want to format JS by `goat_formatter` via goatfmt-plugin,
