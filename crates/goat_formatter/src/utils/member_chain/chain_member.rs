...
pub enum ChainMember<'a, 'b> {
    /// Holds onto a [goat_ast::ast::StaticMemberExpression]
    StaticMember(&'b AstNode<'a, StaticMemberExpression<'a>>),

    /// Holds onto a [goat_ast::ast::CallExpression]
    CallExpression {
        expression: &'b AstNode<'a, CallExpression<'a>>,
        position: CallExpressionPosition,
    },

    /// Holds onto a [goat_ast::ast::ComputedMemberExpression]
    ComputedMember(&'b AstNode<'a, ComputedMemberExpression<'a>>),

    TSNonNullExpression(&'b AstNode<'a, TSNonNullExpression<'a>>),

    /// Any other node that are not [goat_ast::ast::CallExpression] or [goat_ast::ast::StaticMemberExpression]
...
impl ChainMember<'_, '_> {
    ...
    pub fn span(&self) -> goat_span::Span {
        match self {
