...
impl<'a, 'o> JsdocFormatter<'a, 'o> {
    ...
    fn format(mut self, comment: &Comment, source_text: &str) -> Option<FormattedJsdoc<'a>> {
        ...
        let mut effective_tags: Vec<(&goat_jsdoc::parser::JSDocTag<'_>, &str)> =
...
fn sort_tags_by_groups<'a>(
    tags: &'a [goat_jsdoc::parser::JSDocTag<'a>],
) -> Vec<(&'a goat_jsdoc::parser::JSDocTag<'a>, &'a str)> {
    if tags.is_empty() {
        return Vec::new();
    }
    ...
    let normalize =
        |tag: &'a goat_jsdoc::parser::JSDocTag<'a>| (tag, normalize_tag_kind(tag.kind.parsed()));
    ...
    let mut groups: Vec<Vec<(&goat_jsdoc::parser::JSDocTag<'a>, &'a str)>> = Vec::new();
    let mut current_group: Vec<(&goat_jsdoc::parser::JSDocTag<'a>, &'a str)> = Vec::new();
...
fn tag_has_content(tag: &goat_jsdoc::parser::JSDocTag<'_>) -> bool {
...
mod tests {
    ...
    fn fmt_type(type_str: &str) -> Option<String> {
        ...
        let allocator = goat_allocator::Allocator::default();
