...
use goat_ast::{AstKind, ast::Comment};
...
impl<'a> JSDocBuilder<'a> {
    ...
    // - 2) At runtime (usecases like goatlint), reference that flag from the visited node
    //
...
fn should_attach_jsdoc(kind: &AstKind) -> bool {
    matches!(kind,
        // This list order comes from goat_ast/ast_kind.rs
