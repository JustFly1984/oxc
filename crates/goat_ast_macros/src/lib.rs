...
///
/// Firstly, it is a marker for `goat_ast_tools`, to understand that a type is part of the AST.
///
...
/// ### Derive `Ast` trait
///
/// `#[derive(goat_ast_macros::Ast)]` is added to the type.
///
...
/// ### Trait assertions
///
/// `goat_ast_tools` generates code for trait impls where those traits are specified with
...
/// `#[derive(::goat_ast_macros::Ast)]` to the type, to allow the use of helper attributes.
///
/// This attribute should be used on types which are not part of the AST, but are used by `goat_ast_tools`
/// in processing the AST in some way. The purpose of this attribute is to pass data to `goat_ast_tools`.
#[proc_macro_attribute]
pub fn ast_meta(_args: TokenStream, input: TokenStream) -> TokenStream {
    let mut output = TokenStream::from(quote!( #[derive(::goat_ast_macros::Ast)] ));
    output.extend(input);
    output
}
