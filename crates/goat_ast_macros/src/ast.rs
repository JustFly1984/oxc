...
/// Add `#[repr(...)]` and `#[derive(::goat_ast_macros::Ast)]` to enum,
...
/// Add `#[repr(C)]` / `#[repr(transparent)]`, and `#[derive(::goat_ast_macros::Ast)]` to struct,
/// and static assertions for `#[generate_derive]`.
...
/// const _: () = {
///     {
///         trait AssertionTrait: ::goat_span::GetSpan {}
///         impl<T: GetSpan> AssertionTrait for T {}
///     }
/// };
/// ```
///
/// If `GetSpan` is not in scope, or it is not the correct `goat_span::GetSpan`,
/// this will raise a compilation error.
///
/// If any errors e.g. cannot parse `#[generate_derive]`, or unknown traits, just skip them.
/// It is the responsibility of `goat_ast_tools` to raise errors for those.
