...
use goat_allocator::{Address, Allocator, GetAddress, TakeIn, UnstableAddress};
use goat_ast::ast::*;
use goat_ast_visit::{VisitMut, walk_mut};
use goat_diagnostics::GoatDiagnostic;
use goat_parser::Parser;
use goat_semantic::{IsGlobalReference, ReferenceFlags, ScopeFlags, Scoping};
use goat_span::{CompactStr, SPAN, SourceType};
use goat_syntax::identifier::is_identifier_name;
use goat_syntax::node::NodeId;
use goat_syntax::reference::Reference;
...
impl ReplaceGlobalDefinesConfig {
    ...
    pub fn new<S: AsRef<str>>(defines: &[(S, S)]) -> Result<Self, Vec<GoatDiagnostic>> {
    ...
    fn check_key(key: &str) -> Result<IdentifierType, Vec<GoatDiagnostic>> {
        ...
        if parts.len() == 1 {
            if !is_identifier_name(parts[0]) {
                return Err(vec![GoatDiagnostic::error(format!(
    ...
    fn check_value(allocator: &Allocator, source_text: &str) -> Result<(), Vec<GoatDiagnostic>> {
...
impl<'a> ReplaceGlobalDefines<'a> {
    ...
    fn replace_identifier_define_impl(
        &mut self,
        ident: &goat_allocator::Box<'_, IdentifierReference<'_>>,
    ) -> Option<Expression<'a>> {
