...
//! ```bash
//! cargo run -p goat_linter --example linter [filename]
//! ```
...
// Instruction:
// create a `test.js`,
// run `cargo run -p goat_linter --example linter`
// or `cargo watch -x "run -p goat_linter --example linter"`
...
fn main() -> std::io::Result<()> {
    ...
    let mut errors: Vec<GoatDiagnostic> = vec![];
...
fn print_errors(source_text: &str, errors: Vec<GoatDiagnostic>) {
    for error in errors {
...
//   · ─────────
//   ╰────
fn no_debugger(debugger_span: Span) -> GoatDiagnostic {
    GoatDiagnostic::error("`debugger` statement is not allowed").with_label(debugger_span)
}
...
//   ╰────
fn no_empty_pattern(binding_kind: &str, span: Span) -> GoatDiagnostic {
    GoatDiagnostic::error("empty destructuring pattern is not allowed")
