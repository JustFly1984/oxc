...
use goat_allocator::{Allocator, AllocatorPool, CloneIn, TakeIn, Vec as ArenaVec};
use goat_ast::{
...
use goat_data_structures::box_macros::boxed_array;
use goat_diagnostics::GoatDiagnostic;
use goat_estree_tokens::{ESTreeTokenOptionsJS, update_tokens};
use goat_parser::Token;
...
const WEBSITE_BASE_RULES_URL: &str = "https://goatlint.dev/docs/guide/usage/linter/rules";
...
impl Linter {
    ...
    pub(crate) fn run_with_full_source_text_and_disable_directives<'a>(
        ...
        loop {
            ...
            let rules = rules
                .iter()
                .filter(|(rule, _)| {
                    if is_json_file && rule.plugin_name() != "goat" {
                        return false;
                    }
            ...
            #[cfg(debug_assertions)]
            {
                ...
                ctx_host.get_diagnostics(|diagnostics| {
                    ...
                    assert_eq!(
                        ...
                        "Running with and without optimizations produced different diagnostic counts: {} vs {}.\nThis can be caused by a mismatch between the rule definition and generated RuleRunner impl. Try `cargo run -p goat_linter_codegen` to regenerate.",
                        optimized_diagnostics.len(),
                        unoptimized_diagnostics.len()
                    );
    ...
    fn convert_and_call_external_linter(
        ...
        let globals_json = serde_json::to_string(&globals_and_envs).unwrap_or_else(|e| {
            ...
            ctx_host
                .push_diagnostic(Message::new(GoatDiagnostic::error(message), PossibleFixes::None));
            "{}".to_string()
        });
        ...
        match result {
            Ok(diagnostics) => {
                for diagnostic in diagnostics {
                    ...
                    ctx_host.push_diagnostic(Message::new(
                        GoatDiagnostic::error(diagnostic.message)
                            .with_label(span)
