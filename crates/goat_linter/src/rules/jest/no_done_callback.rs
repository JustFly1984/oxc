...
fn no_done_callback(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Function parameter(s) use the `done` argument")
        .with_help("Return a Promise instead of relying on callback parameter")
        .with_label(span)
}

fn use_await_instead_of_callback(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Function parameter(s) use the `done` argument")
