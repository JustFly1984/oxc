...
fn describe_repeat(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Describe block title is used multiple times in the same describe block.")
        .with_help("Change the title of describe block.")
        .with_label(span)
}

fn test_repeat(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Test title is used multiple times in the same describe block.")
