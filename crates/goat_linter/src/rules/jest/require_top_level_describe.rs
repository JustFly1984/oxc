...
fn too_many_describes(max: usize, repeat: &str, span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Require test cases and hooks to be inside a `describe` block")
        .with_help(format!(
...
fn unexpected_test_case(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Require test cases and hooks to be inside a `describe` block")
        .with_help("All test cases must be wrapped in a describe block.")
        .with_label(span)
}

fn unexpected_hook(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Require test cases and hooks to be inside a `describe` block")
