...
fn restricted_chain(chain_call: &str, span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("Use of `{chain_call}` is disallowed")).with_label(span)
}

fn restricted_chain_with_message(chain_call: &str, message: &str, span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("Use of `{chain_call}` is disallowed"))
