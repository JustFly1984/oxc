...
fn snapshot_matcher_too_many_arguments_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("`toMatchSnapshot` takes at most two arguments.")
        .with_help("Pass a hint string, or a property matcher object followed by a hint string.")
        .with_label(span)
}

fn snapshot_missing_hint_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Snapshot is missing a hint.")
        .with_help("Include a hint string to identify this snapshot in the snapshot file.")
        .with_label(span)
}

fn snapshot_hint_must_be_string_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Snapshot hint must be a string literal.")
        .with_help(
