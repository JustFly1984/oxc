...
use goat_semantic::{AstNode, NodeId, ReferenceId};
...
fn non_global_set_timeout_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("`jest.setTimeout` should only be called in a global scope")
        .with_label(span)
}

fn no_multiple_set_timeouts_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Do not call `jest.setTimeout` multiple times")
        .with_help("Only the last call to `jest.setTimeout` will have an effect.")
        .with_label(span)
}

fn no_unorder_set_timeout_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("`jest.setTimeout` should be placed before any other jest methods.")
        .with_label(span)
}
