...
use goat_span::{Span, Str};

use crate::{context::LintContext, fixer::RuleFixer, rule::Rule, utils::get_node_name};

fn use_mock_shorthand(preferred_name: &str, span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Prefer mock resolved/rejected shorthands for promises.")
...
impl Rule for PreferMockPromiseShorthand {
    fn run<'a>(&self, node: &goat_semantic::AstNode<'a>, ctx: &LintContext<'a>) {
