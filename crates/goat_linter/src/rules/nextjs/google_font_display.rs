use goat_ast::{AstKind, ast::JSXElementName};
...
fn font_display_parameter_missing(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn(
...
fn not_recommended_font_display_value(span: Span, font_display_value: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("`{font_display_value}` is not a recommended font-display value."))
        .with_help("See https://nextjs.org/docs/messages/google-font-display")
        .with_label(span)
}
