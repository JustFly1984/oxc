...
fn missing_href_attribute(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Missing `href` attribute for the `a` element.")
        .with_help("Provide an `href` for the `a` element.")
        .with_label(span)
}

fn incorrect_href(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Use of incorrect `href` for the 'a' element.")
        .with_help("Provide a correct `href` for the `a` element.")
        .with_label(span)
}

fn cant_be_anchor(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("The `a` element has `href` and `onClick`.")
