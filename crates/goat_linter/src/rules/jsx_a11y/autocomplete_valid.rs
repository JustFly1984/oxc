...
fn autocomplete_valid_diagnostic(span: Span, autocomplete: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("`{autocomplete}` is not a valid value for `autocomplete`."))
