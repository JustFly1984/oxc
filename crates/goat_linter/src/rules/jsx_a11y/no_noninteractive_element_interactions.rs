...
use goat_ast::ast::JSXElementName;
...
fn diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Non-interactive element should not have event handlers without a role.")
        .with_help(
    ...
    fn run<'a>(&self, node: &AstNode<'a>, ctx: &LintContext<'a>) {
        ...
        for handler in INTERACTIVE_HANDLERS {
            if let Some(prop) = has_jsx_prop_ignore_case(jsx_el, handler) {
                if let goat_ast::ast::JSXAttributeItem::Attribute(attr) = prop {
