...
fn role_has_required_aria_props_diagnostic(span: Span, role: &str, props: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("`{role}` role is missing required aria props `{props}`."))
