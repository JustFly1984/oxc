...
fn missing_alt_prop(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Missing `alt` attribute.")
        .with_help("Must have `alt` prop, either with meaningful text, or an empty string for decorative images.")
        .with_label(span)
}

fn missing_alt_value(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Invalid `alt` value.")
        .with_help(
...
fn aria_label_value(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Missing value for `aria-label` attribute.")
        .with_help("Give `aria-label` a meaningful value. Prever the `alt` attribute over `aria-label` for images.")
        .with_label(span)
}

fn aria_labelled_by_value(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Missing value for `aria-labelledby` attribute.")
        .with_help("Give `aria-labelledby` an ID to a label element. Prefer the `alt` attribute over `aria-labelledby` for images.")
        .with_label(span)
}

fn prefer_alt(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("ARIA used where native HTML could suffice.")
