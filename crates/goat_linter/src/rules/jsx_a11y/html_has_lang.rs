...
fn missing_lang_prop(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Missing lang attribute.")
        .with_help("Add a `lang` attribute to the `html` element whose value represents the primary language of document.")
        .with_label(span)
}

fn missing_lang_value(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Missing value for `lang` attribute")
