...
use goat_semantic::{AstNode, Reference, SymbolId};
...
fn component_display_name_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Component definition is missing display name.")
        .with_help("Add a `displayName` property to the component.")
        .with_label(span)
}

fn context_display_name_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Context definition is missing display name.")
    ...
    fn run_once(&self, ctx: &LintContext) {
        ...
        if ctx.module_record().export_default.is_some() {
            ...
            for stmt in &program.body {
                if let goat_ast::ast::Statement::ExportDefaultDeclaration(export) = stmt {
...
fn is_create_context_call(call: &goat_ast::ast::CallExpression) -> bool {
    call.callee_name().is_some_and(|name| name == "createContext")
}

/// Check if a class extends React.Component or React.PureComponent
fn extends_react_component(class: &goat_ast::ast::Class) -> bool {
...
fn has_display_name_via_semantic(
    ...
    let component_name_str = component_name.map(goat_span::CompactStr::as_str);
...
fn is_anonymous_export_component(
    export: &goat_ast::ast::ExportDefaultDeclaration,
    ignore_transpiler_name: bool,
) -> Option<ReactComponentInfo> {
...
fn is_module_exports_component(
    assign: &goat_ast::ast::AssignmentExpression,
    ignore_transpiler_name: bool,
    check_context_objects: bool,
) -> Option<ReactComponentInfo> {
...
fn has_create_react_class_display_name(
    call: &goat_ast::ast::CallExpression,
    ignore_transpiler_name: bool,
) -> bool {
...
fn class_has_static_display_name(class: &goat_ast::ast::Class) -> bool {
...
fn class_contains_jsx(class: &goat_ast::ast::Class) -> bool {
