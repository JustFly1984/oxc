...
fn missing_key_prop_for_element_in_array(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn(r#"Missing "key" prop for element in array."#).with_label(span)
}

fn missing_key_prop_for_element_in_iterator(iter_span: Span, el_span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn(r#"Missing "key" prop for element in iterator."#)
...
fn key_prop_must_be_placed_before_spread(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn(r#""key" prop must be placed before any `{...spread}`"#)
...
fn duplicate_key_prop(key_value: &str, span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("Duplicate key '{key_value}' found in JSX elements"))
...
fn gen_diagnostic(span: Span, outer: &InsideArrayOrIterator) -> GoatDiagnostic {
    match outer {
