...
fn missing_sandbox_prop(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("An iframe element is missing a sandbox attribute")
        .with_help("Add a `sandbox` attribute to the `iframe` element.")
        .with_label(span)
}

fn invalid_sandbox_prop(span: Span, value: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("An iframe element defines a sandbox attribute with invalid value: {value}"))
...
fn invalid_sandbox_combination_prop(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("An `iframe` element defines a sandbox attribute with both allow-scripts and allow-same-origin which is invalid")
