...
fn no_danger_with_children_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Only set one of `children` or `props.dangerouslySetInnerHTML`")
...
fn is_object_with_prop_name(
    obj_props: &goat_allocator::Vec<'_, ObjectPropertyKind<'_>>,
    prop_name: &str,
) -> bool {
