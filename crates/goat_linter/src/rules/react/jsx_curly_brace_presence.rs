...
fn jsx_curly_brace_presence_unnecessary_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Curly braces are unnecessary here.").with_label(span)
}
fn jsx_curly_brace_presence_necessary_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Curly braces are required here.")
...
pub struct JsxCurlyBracePresence {
    ...
    ///
    /// - `never` will disallow unnecessary curly braces, e.g. this will be preferred: `<Foo>I love goatlint</Foo>`
    /// - `always` will force the usage of curly braces like this, in all cases: `<Foo>{'I love goatlint'}</Foo>`
...
fn test() {
    ...
    let fix = vec![
        ...
        (
            //Note: this case might not fully translate from eslint to goatlint
            "<MyComponent>foo \\n bar</MyComponent>",
            r#"<MyComponent>{"foo \\n bar"}</MyComponent>"#,
            Some(json!([{ "children": "always" }])),
        ),
