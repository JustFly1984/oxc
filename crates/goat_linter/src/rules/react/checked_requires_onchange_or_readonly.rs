...
fn missing_property(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("`checked` should be used with either `onChange` or `readOnly`.")
        .with_help("Add either `onChange` or `readOnly`.")
        .with_label(span)
}

fn exclusive_checked_attribute(checked_span: Span, default_checked_span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Use either `checked` or `defaultChecked`, but not both.")
