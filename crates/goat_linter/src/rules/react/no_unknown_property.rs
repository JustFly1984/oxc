...
fn invalid_prop_on_tag(span: Span, prop: &str, tag: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn("Invalid property found")
        .with_help(format!("Property '{prop}' is only allowed on: {tag}"))
        .with_label(span)
}

fn data_lowercase_required(span: Span, suggested_prop: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn(
...
fn unknown_prop_with_standard_name(span: Span, x1: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn("Unknown property found")
        .with_help(format!("Use '{x1}' instead"))
        .with_label(span)
}

fn unknown_prop(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Unknown property found")
