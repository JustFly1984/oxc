...
fn jsx_no_constructed_context_values(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("The Context `value` prop should not be constructed.")
...
fn is_context_provider(jsx_opening_elem: &JSXOpeningElement, ctx: &LintContext<'_>) -> bool {
    match &jsx_opening_elem.name {
        goat_ast::ast::JSXElementName::MemberExpression(member_expr) => {
            &member_expr.property.name == "Provider"
        }
        name @ (goat_ast::ast::JSXElementName::Identifier(_)
        | goat_ast::ast::JSXElementName::IdentifierReference(_)) => {
...
fn is_constructed_expression(expr: &Expression, ctx: &LintContext) -> bool {
    ...
    match expr {
        ...
        Expression::ChainExpression(chain_expr) => match &chain_expr.expression {
            goat_ast::ast::ChainElement::CallExpression(call) => {
