...
fn missing_type_prop(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("`button` elements must have an explicit `type` attribute.")
        .with_help("Add a `type` attribute to the `button` element.")
        .with_label(span)
}

fn invalid_type_prop(span: Span, allowed_types: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn("`button` elements must have a valid `type` attribute.")
        .with_help(format!(
