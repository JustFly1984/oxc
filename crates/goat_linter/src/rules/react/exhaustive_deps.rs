...
use goat_semantic::{ReferenceId, ScopeId, Semantic, SymbolId};
...
fn missing_callback_diagnostic(hook_name: &str, span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("React hook {hook_name} requires an effect callback."))
        .with_label(span)
        .with_help("Did you forget to pass a callback to the hook?")
        .with_error_code_scope(SCOPE)
}

fn dependency_array_required_diagnostic(hook_name: &str, span: Span) -> GoatDiagnostic {
...
fn unknown_dependencies_diagnostic(hook_name: &str, span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!(
...
fn async_effect_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Effect callbacks are synchronous to prevent race conditions.")
        .with_label(span)
...
fn dependency_array_not_array_literal_diagnostic(hook_name: &str, span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!(
...
fn literal_in_dependency_array_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("The literal is not a valid dependency because it never changes.")
        .with_label(span)
        .with_help("Remove the literal from the array.")
        .with_error_code_scope(SCOPE)
}

fn duplicate_dependency_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("This dependency is specified more than once in the dependency array.")
        .with_label(span)
...
fn infinite_rerender_call_to_set_state_diagnostic(hook_name: &str, span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!(
...
fn ref_accessed_directly_in_effect_cleanup_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("The ref's value `.current` is accessed directly in the effect cleanup function.")
        .with_label(span)
...
impl Rule for ExhaustiveDeps {
    ...
    fn run<'a>(&self, node: &AstNode<'a>, ctx: &LintContext<'a>) {
        ...
        let component_scope_id = {
            match get_enclosing_function(node, ctx).map(goat_semantic::AstNode::kind) {
...
impl Dependency<'_> {
    ...
    fn to_string(&self) -> String {
        std::iter::once(&self.name).chain(self.chain.iter()).map(goat_span::Str::as_str).join(".")
    }
                ...
                if let Some(function_body) = function_body {
                    return is_function_stable(
                        ...
                        declaration
                            ...
                            .map(goat_ast::ast::BindingIdentifier::symbol_id),
                        ctx,
                        component_scope_id,
                        visited,
                    );
...
mod fix {
    ...
    pub fn remove_dependency<'c, 'a: 'c>(
        ...
        codegen.print_expression(&Expression::ArrayExpression(ast_builder.alloc_array_expression(
            deps.span,
            goat_allocator::Vec::from_iter_in(new_deps, &alloc),
        )));
        fixer.replace(deps.span, codegen.into_source_text())
    }
}
