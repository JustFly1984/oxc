...
use goat_allocator::Vec as ArenaVec;
use goat_ast::{
    AstKind,
    ast::{
...
fn is_fragment_with_single_expression(children: &goat_allocator::Vec<'_, JSXChild<'_>>) -> bool {
...
fn has_less_than_two_children(children: &goat_allocator::Vec<'_, JSXChild<'_>>) -> bool {
...
fn is_fragment_with_only_text_and_is_not_child<'a>(
    id: NodeId,
    node: &goat_allocator::Vec<'a, JSXChild<'a>>,
    ctx: &LintContext,
) -> bool {
    if node.len() != 1 {
        return false;
