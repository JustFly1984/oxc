...
fn missing_returns_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Missing JSDoc `@returns` declaration for function.")
        .with_help("Add a `@returns` tag to the JSDoc comment.")
        .with_label(span)
}

fn duplicate_returns_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Duplicate `@returns` tags.")
