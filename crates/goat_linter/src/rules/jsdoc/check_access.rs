...
fn invalid_access_level(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Invalid access level is specified or missing.")
        .with_help("Valid access levels are `package`, `private`, `protected`, and `public`.")
        .with_label(span)
}

fn redundant_access_tags(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn(
