...
use goat_semantic::{JSDoc, JSDocTag};
use goat_span::Span;
...
fn missing_yields(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Missing JSDoc `@yields` declaration for generator function.")
        .with_help("Add `@yields` tag to the JSDoc comment.")
        .with_label(span)
}

fn duplicate_yields(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Duplicate `@yields` tags.")
        .with_help("Remove redundant `@yields` tag.")
        .with_label(span)
}

fn missing_yields_with_generator(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("`@yields` tag is required when using `@generator` tag.")
        .with_help("Add `@yields` tag to the JSDoc comment.")
        .with_label(span)
}
...
impl Rule for RequireYields {
    ...
    fn run<'a>(&self, node: &AstNode<'a>, ctx: &LintContext<'a>) {
        ...
        match node.kind() {
            ...
            //
            // `goat_semantic` add node flag to detect `yield` is used or NOT.
