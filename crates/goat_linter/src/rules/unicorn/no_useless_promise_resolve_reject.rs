...
fn resolve(span: Span, preferred: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("Prefer `{preferred} value` over `{preferred} Promise.resolve(value)`."))
...
fn reject(span: Span, preferred: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("Prefer `throw error` over `{preferred} Promise.reject(error)`."))
