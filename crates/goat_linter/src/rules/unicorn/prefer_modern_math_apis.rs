...
fn prefer_math_abs(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Prefer `Math.abs(x)` over alternatives").with_label(span)
}

fn prefer_math_hypot(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Prefer `Math.hypot(…)` over alternatives").with_label(span)
}

fn prefer_math_log_n(span: Span, good_method: &str, bad_method: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("Prefer `Math.{good_method}(x)` over `{bad_method}`"))
        .with_label(span)
}
