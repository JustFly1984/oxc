...
fn switch_case_break_position_diagnostic(span: Span, keyword: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("Move `{keyword}` inside the block statement.")).with_label(span)
}
