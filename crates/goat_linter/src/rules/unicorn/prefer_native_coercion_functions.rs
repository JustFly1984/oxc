...
fn function(span: Span, called_fn: &str) -> GoatDiagnostic {
...
fn array_callback(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("The arrow function in the callback of the array is equivalent to `Boolean`. Replace the callback with `Boolean`.")
        .with_label(span)
}
