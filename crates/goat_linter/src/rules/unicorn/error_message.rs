...
fn missing_message(ctor_name: &str, span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("Pass a message to the {ctor_name:1} constructor."))
        .with_label(span)
}

fn empty_message(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Error message should not be an empty string.").with_label(span)
}

fn not_string(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Error message should be a string.").with_label(span)
}
