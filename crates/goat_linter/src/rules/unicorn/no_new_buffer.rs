...
fn no_new_buffer_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Use `Buffer.alloc()` or `Buffer.from()` instead of the deprecated `new Buffer()` constructor.")
...
fn determine_buffer_method(new_expr: &goat_ast::ast::NewExpression) -> Option<&'static str> {
