use goat_allocator::{GetAddress, UnstableAddress};
use goat_ast::{
    AstKind,
...
use goat_span::Span;
use goat_syntax::keyword::RESERVED_KEYWORDS;

use crate::{AstNode, context::LintContext, rule::Rule};

fn prefer_string_raw(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn(r"`String.raw` should be used to avoid escaping `\`.").with_label(span)
}
...
impl Rule for PreferStringRaw {
    fn run<'a>(&self, node: &AstNode<'a>, ctx: &LintContext<'a>) {
        ...
        ctx.diagnostic_with_fix(prefer_string_raw(string_literal.span), |fixer| {
            ...
            let before = ctx.source_range(goat_span::Span::new(0, end));
