...
use goat_regular_expression::ast::Term;
use goat_span::{CompactStr, GetSpan, SPAN, Span};

use crate::{AstNode, ast_util::extract_regex_flags, context::LintContext, rule::Rule};

fn string_literal(span: Span, replacement: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("This pattern can be replaced with `{replacement}`."))
        .with_label(span)
}

fn use_replace_all(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Prefer `String#replaceAll()` over `String#replace()` when using a regex with the global flag.")
        .with_label(span)
}
