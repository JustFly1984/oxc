...
fn never_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Remove the `./` prefix from the relative URL.").with_label(span)
}

fn always_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Add a `./` prefix to the relative URL.").with_label(span)
}
...
impl Rule for RelativeUrlStyle {
    ...
    fn run<'a>(&self, node: &AstNode<'a>, ctx: &LintContext<'a>) {
        ...
        if new_expr.arguments.last().is_some_and(goat_ast::ast::Argument::is_spread) {
            return;
        }
