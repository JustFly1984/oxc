...
fn zero_fraction(span: Span, lit: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn("Don't use a zero fraction in the number.")
        .with_help(format!("Replace the number literal with `{lit}`"))
        .with_label(span)
}

fn dangling_dot(span: Span, lit: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn("Don't use a dangling dot in the number.")
        .with_help(format!("Replace the number literal with `{lit}`"))
        .with_label(span)
}
    ...
    fn run<'a>(&self, node: &AstNode<'a>, ctx: &LintContext<'a>) {
        ...
        ctx.diagnostic_with_fix(
            ...
            |fixer| {
                ...
                let token = ctx.source_range(goat_span::Span::new(0, end));
