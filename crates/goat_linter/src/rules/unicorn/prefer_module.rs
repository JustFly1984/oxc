...
fn use_strict_directive_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn(r#"Do not use "use strict" directive."#)
        .with_help("ES modules are always strict mode, so this directive is redundant.")
        .with_label(span)
}

fn global_return_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn(r#""return" should be used inside a function."#).with_label(span)
}

fn common_js_global_diagnostic(span: Span, name: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!(r#"Do not use "{name}"."#))
