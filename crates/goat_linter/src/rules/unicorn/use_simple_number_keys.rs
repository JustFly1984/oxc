...
fn use_simple_number_keys_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Use a simpler number as a property key.")
...
impl Rule for UseSimpleNumberKeys {
    fn run<'a>(&self, node: &AstNode<'a>, ctx: &LintContext<'a>) {
        ...
        let is_property_key = matches!(parent.kind(), AstKind::ObjectProperty(prop) if {
            matches!(&prop.key, goat_ast::ast::PropertyKey::NumericLiteral(k) if k.span == num.span)
        });
