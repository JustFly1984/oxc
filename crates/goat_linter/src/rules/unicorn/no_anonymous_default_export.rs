...
fn no_anonymous_default_export_diagnostic(span: Span, kind: ErrorNodeKind) -> GoatDiagnostic {
    let kind = match kind {
        ErrorNodeKind::Function => "function",
        ErrorNodeKind::Class => "class",
    };

    GoatDiagnostic::warn(format!("This {kind} default export is missing a name"))
