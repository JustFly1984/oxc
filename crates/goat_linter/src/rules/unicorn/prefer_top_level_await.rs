...
fn prefer_top_level_await_over_promise_chain_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Prefer top-level await over using a promise chain.").with_label(span)
}

fn prefer_top_level_await_over_async_iife_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Prefer top-level await over using an async IIFE.").with_label(span)
}

fn prefer_top_level_await_over_async_function_call_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Prefer top-level await over an async function call.")
