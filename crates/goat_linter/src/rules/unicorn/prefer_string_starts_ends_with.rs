...
use goat_regular_expression::ast::{BoundaryAssertionKind, Term};
...
fn starts_with(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Prefer String#startsWith over a regex with a caret.").with_label(span)
}

fn ends_with(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Prefer String#endsWith over a regex with a dollar sign.").with_label(span)
}
