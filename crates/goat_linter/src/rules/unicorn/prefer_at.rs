use goat_syntax::precedence::Precedence;
...
fn prefer_at_diagnostic(span: Span, method: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("Prefer `.at()` over `{method}`."))
...
impl PreferAt {
    ...
    fn check_slice_index_access<'a>(
        call_expr: &CallExpression<'a>,
        computed: &ComputedMemberExpression<'a>,
        computed_node_id: goat_syntax::node::NodeId,
        ctx: &LintContext<'a>,
    ) {
