...
use goat_allocator::{Allocator, GetAddress, UnstableAddress};
use goat_ast::{
    AstBuilder, AstKind,
    ast::{
...
use goat_span::{SPAN, Span};

use crate::{AstNode, context::LintContext, rule::Rule};

fn prefer_keyboard_event_key_diagnostic(span: Span, deprecated_prop: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("Use `.key` instead of `.{deprecated_prop}`"))
...
impl PreferKeyboardEventKey {
    ...
    fn check_static_member_expression<'a>(
        member_expr: &goat_ast::ast::StaticMemberExpression<'a>,
        node: &AstNode<'a>,
        ctx: &LintContext<'a>,
    ) {
    ...
    fn is_reference_to_symbol(
        expr: &Expression,
        symbol_id: goat_semantic::SymbolId,
        ctx: &LintContext,
    ) -> bool {
        match expr {
    ...
    fn report_diagnostic_with_fix<'a>(
        ...
        if !matches!(
            binary.operator,
            goat_ast::ast::BinaryOperator::Equality | goat_ast::ast::BinaryOperator::StrictEquality
        ) {
            ctx.diagnostic(prefer_keyboard_event_key_diagnostic(property_span, property_name));
            return;
        }
...
enum CallbackFunction<'a> {
    Arrow(&'a goat_ast::ast::ArrowFunctionExpression<'a>),
