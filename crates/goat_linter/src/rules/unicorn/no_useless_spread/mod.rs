...
use goat_span::{GetSpan, SPAN, Span};
...
fn spread_in_list(span: Span, arr_or_obj: &str) -> GoatDiagnostic {
...
fn spread_in_arguments(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Using a spread operator here creates a new array unnecessarily.")
        .with_help("Pass arguments directly instead of spreading an array.")
        .with_label(span)
}

fn iterable_to_array(span: Span, ctor_name: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!(
...
fn iterable_to_array_in_for_of(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Using a spread operator here creates a new array unnecessarily.")
        .with_help("`for…of` can iterate over iterable, it's unnecessary to convert to an array.")
        .with_label(span)
}

fn iterable_to_array_in_yield_star(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Using a spread operator here creates a new array unnecessarily.")
        .with_help("`yield*` can delegate to another iterable, so it's unnecessary to convert the iterable to an array.")
        .with_label(span)
}

fn clone(span: Span, is_array: bool, method_name: Option<&str>) -> GoatDiagnostic {
    let noun = if is_array { "array" } else { "object" };
    GoatDiagnostic::warn(format!("Using a spread operator here creates a new {noun} unnecessarily."))
        .with_help(
