...
fn numeric_separators_style_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Invalid group length in numeric value.")
...
impl NumericSeparatorsStyle {
    fn format_number(&self, number: &NumericLiteral) -> String {
        use goat_syntax::number::NumberBase;
    ...
    fn format_bigint(&self, number: &BigIntLiteral, raw: &str) -> String {
        use goat_syntax::number::BigintBase;
