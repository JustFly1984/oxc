...
use goat_ast::ast::AssignmentOperator;
...
fn prefer_add_event_listener_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Prefer `addEventListener()` over their `on`-function counterparts.")
        .with_label(span)
}
