...
fn known_method(span: Span, obj_name: &str, method_name: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("Prefer using `{obj_name}.prototype.{method_name}`."))
        .with_label(span)
}

fn unknown_method(span: Span, obj_name: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("Prefer using method from `{obj_name}.prototype`."))
        .with_label(span)
}
