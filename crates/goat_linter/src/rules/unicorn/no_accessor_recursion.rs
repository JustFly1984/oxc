...
fn no_accessor_recursion_diagnostic(span: Span, kind: &str) -> GoatDiagnostic {
    let method_kind = match kind {
        "setters" => "set",
        _ => "get",
    };
    GoatDiagnostic::warn(format!("Disallow recursive access to `this` within {kind}."))
        .with_help(format!(
            ...
            AstKind::AssignmentExpression(assign_expr) => {
                if let Some(simple_target) = assign_expr.left.as_simple_assignment_target() {
                    let is_target = match simple_target {
                        goat_ast::ast::SimpleAssignmentTarget::ComputedMemberExpression(
                            member_expr,
                        ) => member_expr.span == node.span(),
                        goat_ast::ast::SimpleAssignmentTarget::StaticMemberExpression(
                            member_expr,
                        ) => member_expr.span == node.span(),
                        goat_ast::ast::SimpleAssignmentTarget::PrivateFieldExpression(
                            member_expr,
                        ) => member_expr.span == node.span(),
                        _ => false,
                    };
