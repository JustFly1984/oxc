...
fn invalid_class_name_diagnostic(span: Span, expected: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("Invalid class name, use `{expected}`.")).with_label(span)
}

fn missing_super_call_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Missing call to `super()` in constructor.").with_label(span)
}

fn invalid_name_property_diagnostic(span: Span, name: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("The `name` property should be set to `{name}`.")).with_label(span)
}

fn pass_message_to_super_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Pass the error message to `super()` instead of setting `this.message`.")
        .with_label(span)
}

fn invalid_export_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Exported error name should match error class").with_label(span)
}
...
fn check_class<'a>(class: &Class<'a>, node: &AstNode<'a>, ctx: &LintContext<'a>) {
    ...
    let Some(constructor_method) = constructor else {
        ...
        if let Some(prop) = name_property {
            let span = prop.value.as_ref().map_or(prop.span, goat_span::GetSpan::span);
