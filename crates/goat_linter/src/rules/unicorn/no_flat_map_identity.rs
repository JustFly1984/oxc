...
use goat_ast::ast::{BindingPattern, Expression};
...
fn no_flat_map_identity_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("`.flatMap(x => x)` is equivalent to `.flat()`.")
...
fn is_identity_arrow(expr: &Expression) -> bool {
    ...
    if let Some(stmt) = arrow.body.statements.first() {
        if let goat_ast::ast::Statement::ExpressionStatement(expr_stmt) = stmt {
