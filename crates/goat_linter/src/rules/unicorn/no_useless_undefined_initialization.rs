...
fn no_useless_undefined_initialization_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Unnecessary initialization to `undefined`.")
...
impl Rule for NoUselessUndefinedInitialization {
    fn run<'a>(&self, node: &AstNode<'a>, ctx: &LintContext<'a>) {
        ...
        if decl.kind == goat_ast::ast::VariableDeclarationKind::Const {
            return;
        }
