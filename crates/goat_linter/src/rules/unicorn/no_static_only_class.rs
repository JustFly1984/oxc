use goat_ast::{AstKind, ast::ClassElement};
...
fn no_static_only_class_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Use an object instead of a `class` with only `static` members.")
        .with_label(span)
}
