...
fn array_mutation_diagnostic(span: Span, method: &str) -> GoatDiagnostic {
...
fn object_assign_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Do not call `Object.assign()` immediately after initializing an object.")
        .with_help("Move the properties from `Object.assign()` into the object initializer.")
        .with_label(span)
}

fn object_property_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn(
...
fn set_add_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Do not call `.add()` immediately after initializing a Set.")
        .with_help("Add the element to the Set initializer array.")
        .with_label(span)
}

fn map_set_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Do not call `.set()` immediately after initializing a Map.")
...
fn expression_references_variable(expr: &Expression<'_>, var_name: &str) -> bool {
    match expr.get_inner_expression() {
        ...
        Expression::UpdateExpression(update) => {
            if let goat_ast::ast::SimpleAssignmentTarget::AssignmentTargetIdentifier(id) =
                &update.argument
            {
                id.name == var_name
            } else {
