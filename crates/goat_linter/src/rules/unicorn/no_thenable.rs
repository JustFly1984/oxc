...
fn object(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Do not add `then` to an object.")
        .with_help("If an object is defined as 'thenable', once it's accidentally used in an await expression, it may cause problems")
        .with_label(span)
}

fn export(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Do not export `then`.")
        .with_help("If an object is defined as 'thenable', once it's accidentally used in an await expression, it may cause problems")
        .with_label(span)
}

fn class(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Do not add `then` to a class.")
        .with_help("If an object is defined as 'thenable', once it's accidentally used in an await expression, it may cause problems")
        .with_label(span)
}
...
fn check_expression(expr: &Expression, ctx: &LintContext<'_>) -> Option<goat_span::Span> {
    match expr {
