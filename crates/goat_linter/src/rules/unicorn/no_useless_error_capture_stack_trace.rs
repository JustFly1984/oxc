...
use goat_semantic::{IsGlobalReference, SymbolId};
use goat_span::Span;
...
fn no_useless_error_capture_stack_trace_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Do not use `Error.captureStackTrace(…)` in the constructor of an Error subclass")
...
fn is_error_class(class: &goat_ast::ast::Class, ctx: &LintContext) -> bool {
