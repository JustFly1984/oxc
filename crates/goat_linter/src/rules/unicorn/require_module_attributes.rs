...
fn require_module_attributes_diagnostic(span: Span, import_type: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("{import_type} with empty attribute list is not allowed."))
        .with_label(span)
...
fn fix_empty_with_property(
    ...
    properties: &goat_allocator::Vec<'_, goat_ast::ast::ObjectPropertyKind<'_>>,
) -> RuleFix {
