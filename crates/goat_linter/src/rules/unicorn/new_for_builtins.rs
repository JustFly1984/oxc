...
fn enforce(span: Span, fn_name: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("Use `new {fn_name}()` instead of `{fn_name}()`")).with_label(span)
}

fn disallow(span: Span, fn_name: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("Use `{fn_name}()` instead of `new {fn_name}()`")).with_label(span)
}
