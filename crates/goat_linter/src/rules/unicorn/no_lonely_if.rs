...
fn no_lonely_if_diagnostic(if_stmt_span: Span, parent_if_stmt_span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Unexpected `if` as the only statement in a `if` block without `else`.")
...
fn needs_parentheses(expr: &Expression) -> bool {
    matches!(
        ...
        Expression::LogicalExpression(e) if matches!(e.operator, goat_syntax::operator::LogicalOperator::Or | goat_syntax::operator::LogicalOperator::Coalesce)
    ) || matches!(
        expr.get_inner_expression(),
