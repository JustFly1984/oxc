...
fn iterable_accepting_diagnostic(span: Span, description: &str) -> GoatDiagnostic {
...
fn for_of_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("`for…of` can iterate over an iterable, `.toArray()` is unnecessary.")
        .with_label(span)
}

fn yield_star_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("`yield*` can delegate to an iterable, `.toArray()` is unnecessary.")
        .with_label(span)
}

fn spread_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Spread works on iterables, `.toArray()` is unnecessary.").with_label(span)
}

fn iterator_method_diagnostic(span: Span, method: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!(
