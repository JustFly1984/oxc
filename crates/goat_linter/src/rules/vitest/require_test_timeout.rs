...
fn test_missing_timeout(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Test is missing a timeout.")
        .with_help(
...
fn config_missing_timeout_object(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("`vi.setConfig()` is missing a `testTimeout` property.")
        .with_help("Pass an object with a `testTimeout` property to `vi.setConfig()`.")
        .with_label(span)
}

fn test_options_missing_timeout_property(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Test options object is missing a `timeout` property.")
        .with_help("Add a `timeout` property to the options object.")
        .with_label(span)
}

fn timeout_must_be_a_number(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Timeout must be a number.")
        .with_help("Use a non-negative numeric literal for the timeout value.")
        .with_label(span)
}

fn timeout_must_be_non_negative(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Timeout must not be negative.")
        .with_help("Use a non-negative numeric literal for the timeout value.")
        .with_label(span)
}
