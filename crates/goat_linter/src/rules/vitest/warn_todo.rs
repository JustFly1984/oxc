...
fn warn_todo_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("The use of `.todo` is not recommended.")
...
fn test() {
    ...
    /*
     * Currently the responsible to set what frameworks are active or not is not `with_vitest_plugin` or goatlint config.
