...
fn require_awaited_expect_poll_diagnostic(span: Span, member_name: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("`expect.{member_name}` must be awaited or returned"))
        .with_help(format!("Add `await` to the `expect.{member_name}` call."))
        .with_label(span)
}

fn require_awaited_expect_poll_return_diagnostic(span: Span, member_name: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("`expect.{member_name}` must be awaited or returned as the last expression"))
