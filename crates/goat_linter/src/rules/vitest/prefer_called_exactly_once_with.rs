...
use goat_allocator::Vec as OxcVec;
use goat_ast::{
    AstKind,
...
fn prefer_called_exactly_once_with_diagnostic(
    ...
    GoatDiagnostic::warn("Prefer `toHaveBeenCalledExactlyOnceWith` over `toHaveBeenCalledOnce` and `toHaveBeenCalledWith` on the same target.")
        .with_labels([
...
enum TestCallExpression<'a> {
    TestBlock(&'a goat_allocator::Vec<'a, Statement<'a>>),
    MockReset,
