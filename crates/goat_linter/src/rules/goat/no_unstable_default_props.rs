...
fn no_unstable_default_props_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Unstable value used as default prop")
...
impl Rule for NoUnstableDefaultProps {
    fn run<'a>(&self, node: &AstNode<'a>, ctx: &LintContext<'a>) {
        ...
        let goat_ast::ast::BindingPattern::ObjectPattern(obj_pattern) = &first_param.pattern else {
            return;
        };

        for prop in &obj_pattern.properties {
            if let goat_ast::ast::BindingPattern::AssignmentPattern(assign) = &prop.value
