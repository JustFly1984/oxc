...
fn invalid_package_json_license_diagnostic(span: goat_span::Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("The `license` field in package.json must be a valid SPDX expression, `UNLICENSED`, or `SEE LICENSE IN <file>`.")
...
fn non_string_package_json_license_diagnostic(span: goat_span::Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("The `license` field in package.json must be a string.")
