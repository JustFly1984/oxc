...
use goat_span::{Span, ident::MATH};

use crate::{AstNode, context::LintContext, fixer::RuleFixer, rule::Rule};

fn approx_constant_diagnostic(span: Span, method_name: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("Approximate value of `{method_name}` found."))
