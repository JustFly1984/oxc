...
use goat_ast::{AstKind, ast::ObjectExpression};
...
fn sort_objects_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Object keys should be sorted alphabetically.").with_label(span)
}
...
impl Rule for SortObjects {
    ...
    fn run<'a>(&self, node: &AstNode<'a>, ctx: &LintContext<'a>) {
        ...
        let keyed: Vec<(usize, String)> = obj
            ...
            .enumerate()
            .filter_map(|(i, prop)| {
                let goat_ast::ast::ObjectPropertyKind::ObjectProperty(p) = prop else {
                    return None;
                };
