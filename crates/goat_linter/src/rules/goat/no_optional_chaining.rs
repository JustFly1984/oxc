...
fn no_optional_chaining_diagnostic(span: Span, help: &str) -> GoatDiagnostic {
    if help.is_empty() {
        GoatDiagnostic::warn("Optional chaining is not allowed.").with_label(span)
    } else {
        GoatDiagnostic::warn("Optional chaining is not allowed.")
