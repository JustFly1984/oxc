...
fn invalid_package_json_engines_diagnostic(span: goat_span::Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("The `engines` field in package.json is invalid.")
        .with_help("Use an object with runtime names as keys and semver ranges as values, e.g. `{ \"node\": \">=18\" }`.")
        .with_label(span)
}

fn invalid_engine_value_diagnostic(key: &str, span: goat_span::Span) -> GoatDiagnostic {
