...
fn json_parse_validation_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Validate `JSON.parse()` results before using them.")
        .with_help(
...
fn is_immediately_validated(node_id: goat_syntax::node::NodeId, ctx: &LintContext<'_>) -> bool {
...
fn is_validation_call(call_expr: &CallExpression) -> bool {
    match &call_expr.callee {
        goat_ast::ast::Expression::Identifier(ident) => {
        ...
        goat_ast::ast::Expression::ChainExpression(chain) => chain
            .expression
            .as_member_expression()
            .and_then(goat_ast::ast::MemberExpression::static_property_name)
            .is_some_and(|name| matches!(name, "safeParse" | "parse")),
        _ => false,
    }
}
    ...
    fn call_uses_target_identifier(&self, call_expr: &CallExpression<'_>) -> bool {
        call_expr.arguments.iter().filter_map(|argument| argument.as_expression()).any(
            |expression| match expression.get_inner_expression() {
                goat_ast::ast::Expression::Identifier(ident) => ident.name == self.name,
                _ => false,
            },
        )
    }
