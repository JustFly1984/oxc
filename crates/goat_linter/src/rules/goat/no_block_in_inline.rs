...
use goat_ast::{AstKind, ast::JSXOpeningElement};
...
fn no_block_in_inline_diagnostic(span: Span, block: &str, inline: &str) -> GoatDiagnostic {
...
fn get_html_element_name<'a>(opening_element: &JSXOpeningElement<'a>) -> Option<&'a str> {
    let goat_ast::ast::JSXElementName::Identifier(identifier) = &opening_element.name else {
        return None;
    };
