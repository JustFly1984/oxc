...
fn unsorted_keys_diagnostic(key: &str, span: goat_span::Span) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("Entries in `{key}` are not in lexicographical order."))
        .with_help("Sort the collection consistently to reduce noisy diffs.")
        .with_label(span)
}

fn unsorted_scripts_diagnostic(span: goat_span::Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Entries in `scripts` are not in lifecycle-aware lexicographical order.")
        .with_help(
...
impl Rule for PackageJsonSortCollections {
    ...
    fn run_once(&self, ctx: &LintContext<'_>) {
        ...
        let file_span = goat_span::Span::new(0, source_text.len() as u32);
        let diagnostic = if path_targets_scripts(&first_unsorted_path) {
            unsorted_scripts_diagnostic(goat_span::Span::new(0, 1))
        } else {
            unsorted_keys_diagnostic(&first_unsorted_path, goat_span::Span::new(0, 1))
        };
