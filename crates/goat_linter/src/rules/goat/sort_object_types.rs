...
fn sort_object_types_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("TypeScript object type members should be sorted alphabetically.")
        .with_label(span)
}
...
fn member_key(member: &goat_ast::ast::TSSignature<'_>, ctx: &LintContext<'_>) -> Option<String> {
    match member {
        goat_ast::ast::TSSignature::TSPropertySignature(prop) => match &prop.key {
        ...
        goat_ast::ast::TSSignature::TSMethodSignature(method) => match &method.key {
            goat_ast::ast::PropertyKey::StaticIdentifier(ident) => Some(ident.name.to_string()),
