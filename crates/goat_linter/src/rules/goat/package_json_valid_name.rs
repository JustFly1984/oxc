...
fn invalid_package_json_name_diagnostic(complaints: &str, span: goat_span::Span) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("Invalid npm package name: {complaints}."))
        .with_help("Use a valid npm package name, or mark the package as private if it is not meant to be published.")
        .with_label(span)
}

fn non_string_package_json_name_diagnostic(span: goat_span::Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("The `name` field in package.json must be a string.")
