...
fn no_nested_components_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Component definition inside another component")
    ...
    fn run<'a>(&self, node: &AstNode<'a>, ctx: &LintContext<'a>) {
        ...
        let (fn_name_span, has_jsx) = match node.kind() {
            ...
            AstKind::VariableDeclarator(decl) => {
                ...
                let goat_ast::ast::BindingPattern::BindingIdentifier(id) = &decl.id else {
                    return;
                };
                if !id.name.starts_with(|c: char| c.is_ascii_uppercase()) {
                    return;
                ...
                let is_fn = matches!(
                    init,
                    goat_ast::ast::Expression::ArrowFunctionExpression(_)
                        | goat_ast::ast::Expression::FunctionExpression(_)
                );
                if !is_fn {
                    return;
                }
        ...
        for ancestor_id in ctx.nodes().ancestor_ids(node.id()).skip(1) {
            ...
            match ancestor.kind() {
                ...
                AstKind::VariableDeclarator(decl) => {
                    if let goat_ast::ast::BindingPattern::BindingIdentifier(id) = &decl.id
                        && id.name.starts_with(|c: char| c.is_ascii_uppercase())
