...
fn detect_disable_mustache_escape_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Mustache escaping is being disabled")
...
impl Rule for DetectDisableMustacheEscape {
    fn run<'a>(&self, node: &AstNode<'a>, ctx: &LintContext<'a>) {
        ...
        let key_name = match &prop.key {
            goat_ast::ast::PropertyKey::StaticIdentifier(ident) => &ident.name,
            _ => return,
        };
