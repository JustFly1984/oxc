...
fn high_entropy_diagnostic(span: Span, token: &str, entropy: f64) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("Found a high-entropy string `{token}` ({entropy:.2})."))
        .with_help("Move the secret out of source code or replace it with a safe test value.")
        .with_label(span)
}

fn pattern_match_diagnostic(span: Span, name: &str, matched: &str) -> GoatDiagnostic {
...
fn is_module_path_string(
    ...
    match ctx.nodes().parent_kind(node_id) {
        ...
        AstKind::CallExpression(call_expr) => {
            matches!(&call_expr.callee, goat_ast::ast::Expression::Identifier(ident) if ident.name == "require")
                && call_expr
                    .arguments
                    .first()
                    .and_then(goat_ast::ast::Argument::as_expression)
