...
fn detect_no_csrf_before_method_override_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("CSRF middleware found before method-override")
...
impl Rule for DetectNoCsrfBeforeMethodOverride {
    fn run_once(&self, ctx: &LintContext<'_>) {
        ...
        for stmt in &ctx.semantic().nodes().program().body {
            let goat_ast::ast::Statement::ExpressionStatement(expr_stmt) = stmt else {
                continue;
            };
