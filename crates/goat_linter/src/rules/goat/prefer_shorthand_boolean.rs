...
use goat_ast::ast::JSXAttributeItem;
...
fn prefer_shorthand_boolean_diagnostic(span: Span, name: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("Use shorthand boolean attribute: `{name}` instead of `{name}={{true}}`"))
...
impl Rule for PreferShorthandBoolean {
    fn run<'a>(&self, node: &AstNode<'a>, ctx: &LintContext<'a>) {
        ...
        for attr in &element.attributes {
            ...
            let goat_ast::ast::JSXAttributeValue::ExpressionContainer(container) = value else {
                continue;
            };

            let goat_ast::ast::JSXExpression::BooleanLiteral(lit) = &container.expression else {
                continue;
            };
