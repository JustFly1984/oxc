...
fn prefer_object_diagnostic(span: goat_span::Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Prefer an object locator for a repository.")
        .with_help("Use `{ \"type\": \"git\", \"url\": \"...\" }` for the `repository` field.")
        .with_label(span)
}

fn prefer_shorthand_diagnostic(span: goat_span::Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Prefer a shorthand locator for a supported repository provider.")
