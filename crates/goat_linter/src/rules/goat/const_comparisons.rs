...
use goat_syntax::operator::{BinaryOperator, LogicalOperator};

use crate::{AstNode, context::LintContext, rule::Rule, utils::is_same_expression};

fn redundant_left_hand_side(left_span: Span, right_span: Span, help: String) -> GoatDiagnostic {
    GoatDiagnostic::warn("Left-hand side of `&&` operator has no effect.")
        .with_help(help)
        .with_labels([
...
fn redundant_right_hand_side(right_span: Span, left_span: Span, help: String) -> GoatDiagnostic {
    GoatDiagnostic::warn("Right-hand side of `&&` operator has no effect.")
        .with_help(help)
        .with_labels([
...
fn impossible(span: Span, span1: Span, x2: &str, x3: &str, x4: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn("Unexpected constant comparison").with_help(x4.to_string()).with_labels([
...
fn constant_comparison_diagnostic(
    ...
    let diagnostic =
        GoatDiagnostic::warn(format!("This comparison will always evaluate to {evaluates_to}"))
            .with_help(help)
            .with_label(span);
...
fn identical_expressions_logical_operator(left_span: Span, right_span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Both sides of the logical operator are the same")
...
fn equivalent_expressions_logical_operator(left_span: Span, right_span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Both sides of the logical operator are equivalent")
...
fn complementary_expressions_logical_operator(
    ...
    GoatDiagnostic::warn("Unexpected constant comparison")
