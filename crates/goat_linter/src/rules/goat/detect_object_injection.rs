...
fn variable_assigned_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Variable Assigned to Object Injection Sink")
        .with_help(
...
fn function_call_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Function Call Object Injection Sink")
        .with_help("Avoid passing dynamically indexed object access into security-sensitive flows without validating the key first.")
        .with_label(span)
}

fn generic_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Generic Object Injection Sink")
        .with_help(
            "Avoid indexing objects with untrusted dynamic keys without validating the key first.",
        )
        .with_label(span)
