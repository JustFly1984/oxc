...
fn bad_comparison_sequence_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Bad comparison sequence").with_help("Comparison result should not be used directly as an operand of another comparison. If you need to compare three or more operands, you should connect each comparison operation with logical AND operator (`&&`)").with_label(span)
}
