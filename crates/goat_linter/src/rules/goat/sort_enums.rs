...
fn sort_enums_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Enum members should be sorted alphabetically.").with_label(span)
}
        ...
        let members: Vec<(usize, String)> = enum_decl
            ...
            .map(|(i, member)| {
                let key = match &member.id {
                    goat_ast::ast::TSEnumMemberName::Identifier(ident) => ident.name.to_string(),
                    goat_ast::ast::TSEnumMemberName::String(lit) => lit.value.to_string(),
