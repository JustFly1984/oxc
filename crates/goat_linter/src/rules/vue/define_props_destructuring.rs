use goat_ast::{AstKind, ast::BindingPattern};
...
fn prefer_destructuring_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Prefer destructuring from `defineProps` directly.").with_label(span)
}

fn avoid_destructuring_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Avoid destructuring from `defineProps`.").with_label(span)
}

fn avoid_with_defaults_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Avoid using `withDefaults` with destructuring.").with_label(span)
}
