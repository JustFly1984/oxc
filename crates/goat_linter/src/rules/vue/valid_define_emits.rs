...
fn has_type_and_arguments_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("`defineEmits` has both a type-only emit and an argument.")
        .with_help("remove the argument for better type inference.")
        .with_label(span)
}

fn called_multiple_times(span: Span, second_span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("`defineEmits` has been called multiple times.")
...
fn events_not_defined(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Custom events are not defined.")
        .with_help("Define at least one event in `defineEmits`.")
        .with_label(span)
}

fn referencing_locally(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("`defineEmits` is referencing locally declared variables.")
        .with_help("inline the variable or import it from another module.")
        .with_label(span)
}

fn define_in_both(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Custom events are defined in both `defineEmits` and `export default {}`.")
