...
fn no_import_compiler_macros_diagnostic(span: Span, name: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("'{name}' is a compiler macro and doesn't need to be imported."))
        .with_help("Remove the import statement for this macro.")
        .with_label(span)
}

fn invalid_import_compiler_macros_diagnostic(span: Span, name: &str) -> GoatDiagnostic {
