...
fn no_export(span: Span, specifier_name: &str, namespace_name: &str) -> GoatDiagnostic {
...
fn computed_reference(span: Span, namespace_name: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!(
...
fn assignment(span: Span, namespace_name: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("Assignment to member of namespace {namespace_name:?}.'"))
...
fn check_binding_exported(
    name: &str,
    get_diagnostic: impl FnOnce() -> GoatDiagnostic,
    module: &ModuleRecord,
    ctx: &LintContext<'_>,
) {
