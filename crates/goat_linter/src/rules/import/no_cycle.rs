...
fn no_cycle_diagnostic(span: Span, stack: &[(CompactStr, PathBuf)], cwd: &Path) -> GoatDiagnostic {
    let cycle_description = format_cycle(stack, cwd);
    GoatDiagnostic::warn("Dependency cycle detected")
...
fn self_referencing_cycle_diagnostic(span: Span, is_import: bool) -> GoatDiagnostic {
    GoatDiagnostic::warn("Dependency cycle detected")
