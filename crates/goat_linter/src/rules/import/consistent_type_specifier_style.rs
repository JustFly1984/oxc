...
use goat_codegen::{Context, Gen};
use goat_diagnostics::GoatDiagnostic;
use goat_macros::declare_goat_lint;
use goat_span::{FileExtension, GetSpan, SPAN, Span};
...
fn use_top_level_for_declaration_file_import_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn(
...
fn consistent_type_specifier_style_diagnostic(span: Span, mode: &Mode) -> GoatDiagnostic {
    ...
    GoatDiagnostic::warn(warn_msg).with_help(help_msg).with_label(span)
}
...
fn gen_value_import_declaration<'c, 'a: 'c>(
    ...
    let import_declaration = ast_builder.alloc_import_declaration(
        SPAN,
        Some(goat_allocator::Vec::from_iter_in(specifiers, &alloc)),
