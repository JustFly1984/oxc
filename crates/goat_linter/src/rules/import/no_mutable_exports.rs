...
use goat_semantic::ReferenceId;
use goat_span::Span;

use crate::{context::LintContext, rule::Rule};

fn no_mutable_exports_diagnostic(span: Span, kind: VariableDeclarationKind) -> GoatDiagnostic {
    let kind_str = if kind == VariableDeclarationKind::Var { "var" } else { "let" };
    GoatDiagnostic::warn(format!("Exporting mutable '{kind_str}' binding, use 'const' instead."))
