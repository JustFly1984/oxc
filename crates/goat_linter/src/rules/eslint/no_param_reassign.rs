...
use goat_semantic::{AstNode, NodeId, Reference};
...
fn assignment_to_param_diagnostic(name: &str, span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("Assignment to function parameter '{name}'."))
        .with_help("Consider using a different variable to avoid unintended side effects on the parameter.")
        .with_label(span)
}

fn assignment_to_param_property_diagnostic(name: &str, span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("Assignment to property of function parameter '{name}'."))
