...
use goat_syntax::operator::{AssignmentOperator, BinaryOperator, UnaryOperator};
...
fn boolean_coercion_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Unexpected implicit coercion to boolean")
        .with_help("Use `Boolean(value)` instead")
        .with_label(span)
}

fn number_coercion_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Unexpected implicit coercion to number")
        .with_help("Use `Number(value)` instead")
        .with_label(span)
}

fn string_coercion_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Unexpected implicit coercion to string")
    ...
    match expr.without_parentheses() {
        Expression::CallExpression(call) => {
            ...
            if let Expression::ChainExpression(chain) = callee
                && let goat_ast::ast::ChainElement::StaticMemberExpression(member) =
                    &chain.expression
            {
                return matches!(member.property.name.as_str(), "indexOf" | "lastIndexOf");
            }
        ...
        Expression::ChainExpression(chain) => {
            if let goat_ast::ast::ChainElement::CallExpression(call) = &chain.expression
