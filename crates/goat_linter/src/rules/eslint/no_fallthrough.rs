...
fn no_fallthrough_case_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Expected a `break` statement before `case`.")
        .with_help("Use a `break` statement to prevent fallthrough, or add a comment to indicate intentional fallthrough.")
        .with_label(span)
}

fn no_fallthrough_default_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Expected a `break` statement before `default`.")
        .with_help("Use a `break` statement to prevent fallthrough, or add a comment to indicate intentional fallthrough.")
        .with_label(span)
}

fn no_unused_fallthrough_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn(
...
impl NoFallthrough {
    ...
    fn is_comment_fall_through(&self, comment: &str) -> bool {
        if comment.starts_with("goatlint-") || comment.starts_with("eslint-") {
            return false;
        }
