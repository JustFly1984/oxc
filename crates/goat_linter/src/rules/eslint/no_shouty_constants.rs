...
fn no_shouty_constants_diagnostic(span: Span, name: &str) -> GoatDiagnostic {
...
impl Rule for NoShoutyConstants {
    fn run<'a>(&self, node: &AstNode<'a>, ctx: &LintContext<'a>) {
        ...
        if declarator.kind != goat_ast::ast::VariableDeclarationKind::Const {
            return;
        }

        let goat_ast::ast::BindingPattern::BindingIdentifier(ident) = &declarator.id else {
            return;
        };
