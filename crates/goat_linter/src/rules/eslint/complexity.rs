...
use goat_semantic::ScopeFlags;
use goat_span::{GetSpan, Span};
use goat_syntax::operator::AssignmentOperator;
...
fn complexity_diagnostic(span: Span, name: &str, complexity: usize, max: usize) -> GoatDiagnostic {
...
impl Visit<'_> for ComplexityVisitor {
    fn visit_catch_clause(&mut self, it: &goat_ast::ast::CatchClause<'_>) {
        self.complexity += 1;
        walk::walk_catch_clause(self, it);
    }

    fn visit_conditional_expression(&mut self, it: &goat_ast::ast::ConditionalExpression<'_>) {
        self.complexity += 1;
        walk::walk_conditional_expression(self, it);
    }

    fn visit_logical_expression(&mut self, it: &goat_ast::ast::LogicalExpression<'_>) {
        self.complexity += 1;
        walk::walk_logical_expression(self, it);
    }

    fn visit_for_statement(&mut self, it: &goat_ast::ast::ForStatement<'_>) {
        self.complexity += 1;
        walk::walk_for_statement(self, it);
    }

    fn visit_for_in_statement(&mut self, it: &goat_ast::ast::ForInStatement<'_>) {
        self.complexity += 1;
        walk::walk_for_in_statement(self, it);
    }

    fn visit_for_of_statement(&mut self, it: &goat_ast::ast::ForOfStatement<'_>) {
        self.complexity += 1;
        walk::walk_for_of_statement(self, it);
    }

    fn visit_if_statement(&mut self, it: &goat_ast::ast::IfStatement<'_>) {
        self.complexity += 1;
        walk::walk_if_statement(self, it);
    }

    fn visit_while_statement(&mut self, it: &goat_ast::ast::WhileStatement<'_>) {
        self.complexity += 1;
        walk::walk_while_statement(self, it);
    }

    fn visit_do_while_statement(&mut self, it: &goat_ast::ast::DoWhileStatement<'_>) {
        self.complexity += 1;
        walk::walk_do_while_statement(self, it);
    }

    fn visit_assignment_pattern(&mut self, it: &goat_ast::ast::AssignmentPattern<'_>) {
        self.complexity += 1;
        walk::walk_assignment_pattern(self, it);
    }

    fn visit_formal_parameter(&mut self, it: &goat_ast::ast::FormalParameter<'_>) {
    ...
    fn visit_switch_case(&mut self, it: &goat_ast::ast::SwitchCase<'_>) {
    ...
    fn visit_switch_statement(&mut self, it: &goat_ast::ast::SwitchStatement<'_>) {
    ...
    fn visit_assignment_expression(&mut self, it: &goat_ast::ast::AssignmentExpression<'_>) {
        if matches!(
            it.operator,
    ...
    fn visit_member_expression(&mut self, it: &goat_ast::ast::MemberExpression<'_>) {
        if it.optional() {
            self.complexity += 1;
        }
    ...
    fn visit_call_expression(&mut self, it: &goat_ast::ast::CallExpression<'_>) {
        if it.optional {
            self.complexity += 1;
        }
    ...
    fn visit_arrow_function_expression(&mut self, it: &goat_ast::ast::ArrowFunctionExpression<'_>) {
        if !self.has_entered_complexity_evaluation {
            self.has_entered_complexity_evaluation = true;
    ...
    fn visit_static_block(&mut self, it: &goat_ast::ast::StaticBlock<'_>) {
        if !self.has_entered_complexity_evaluation {
            self.has_entered_complexity_evaluation = true;
    ...
    fn visit_property_definition(&mut self, it: &goat_ast::ast::PropertyDefinition<'_>) {
        if self.has_entered_complexity_evaluation {
            // Visit all other nodes except for value expression if part of another
            // function / static block's complexity evaluation
            let kind = goat_ast::AstKind::PropertyDefinition(self.alloc(it));
