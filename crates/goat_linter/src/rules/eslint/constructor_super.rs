...
fn missing_super_all(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Expected to call `super()`.")
        .with_help("Add a `super()` call to the constructor")
        .with_label(span)
}

fn missing_super_some(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Lacked a call of `super()` in some code paths.")
        .with_help("Ensure `super()` is called in all code paths")
        .with_label(span)
}

fn duplicate_super(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Unexpected duplicate `super()`.")
        .with_help("Remove the duplicate `super()` call")
        .with_label(span)
}

fn bad_super(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Unexpected `super()` because `super` is not a constructor.")
        ...
        neighbors_filtered_by_edge_weight(
            ...
            &mut |block_id, (): ()| {
                ...
                for instruction in block.instructions() {
                    ...
                    if matches!(
                        instruction.kind,
                        goat_cfg::InstructionKind::Return(_) | goat_cfg::InstructionKind::Throw
                    ) {
                        hit_exit = true;
                        continue;
                    }
    ...
    fn analyze_super_paths(
        ...
        while let Some(block_id) = worklist.pop_front() {
            ...
            let has_exit = block.instructions().iter().any(|inst| {
                matches!(
                    inst.kind,
                    goat_cfg::InstructionKind::Return(_)
                        | goat_cfg::InstructionKind::Throw
                        | goat_cfg::InstructionKind::ImplicitReturn
                )
            });
            ...
            if has_exit {
                let is_acceptable_exit = block.instructions().iter().any(|inst| {
                    matches!(
                        inst.kind,
                        goat_cfg::InstructionKind::Throw
                            | goat_cfg::InstructionKind::Return(
                                goat_cfg::ReturnInstructionKind::NotImplicitUndefined
                            )
                    )
                });
            ...
            for edge in cfg.graph.edges_directed(block_id, Direction::Outgoing) {
                ...
                match edge.weight() {
                    ...
                    EdgeType::Error(goat_cfg::ErrorEdgeKind::Explicit) => {
                    ...
                    | EdgeType::Error(goat_cfg::ErrorEdgeKind::Implicit) => {}
