...
fn class_methods_use_this_diagnostic(span: Span, name: Option<Cow<'_, str>>) -> GoatDiagnostic {
    let method_name_str = name.map_or(String::new(), |name| format!(" `{name}`"));
    GoatDiagnostic::warn(format!("Expected method{method_name_str} to have this."))
...
impl Visit<'_> for ThisFinder {
    ...
    fn visit_function(
        &mut self,
        _it: &goat_ast::ast::Function<'_>,
        _flags: goat_semantic::ScopeFlags,
    ) {
    }

    fn visit_static_block(&mut self, _it: &goat_ast::ast::StaticBlock<'_>) {}
