...
use goat_ast::ast::{IfStatement, Statement};
use goat_diagnostics::GoatDiagnostic;
use goat_macros::declare_goat_lint;
use goat_span::{GetSpan, Span};

fn no_lonely_if_diagnostic(lonely_if: &IfStatement) -> GoatDiagnostic {
    let span = Span::sized(lonely_if.span.start, 2);
    GoatDiagnostic::warn("Unexpected `if` as the only statement in an `else` block")
