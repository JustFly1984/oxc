...
use goat_allocator::Box;
use goat_ast::AstKind;
use goat_ast::ast::{Expression, ObjectExpression, ObjectPropertyKind, PropertyKind};
...
fn prefer_object_spread_diagnostic(span: Span, for_use_literal: bool) -> GoatDiagnostic {
    let help_message = if for_use_literal {
        "Use an object literal instead of `Object.assign`. eg: `{ foo: bar }`."
    } else {
        "Use an object spread instead of `Object.assign` eg: `{ ...foo }`."
    };
    GoatDiagnostic::warn("Disallow using `Object.assign` with an object literal as the first argument and prefer the use of object spread instead")
