...
use goat_semantic::{NodeId, Reference, SymbolId};
use goat_span::{GetSpan, Span};

use crate::{AstNode, context::LintContext, rule::Rule};

fn no_unmodified_loop_condition_diagnostic(name: &str, span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("'{name}' is not modified in this loop.")).with_label(span)
}
...
impl NoUnmodifiedLoopCondition {
    ...
    fn get_enclosing_function_declaration_symbol_id(
        ...
        while current_id != NodeId::ROOT {
            ...
            {
                return function.id.as_ref().map(goat_ast::ast::BindingIdentifier::symbol_id);
            }
