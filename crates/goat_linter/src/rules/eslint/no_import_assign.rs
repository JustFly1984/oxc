...
fn no_import_assign_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Do not assign to imported bindings")
        ...
        if let Some(specifiers) = &import_decl.specifiers {
            for specifier in specifiers {
                ...
                let is_namespace_specifier = matches!(
                    specifier,
                    goat_ast::ast::ImportDeclarationSpecifier::ImportNamespaceSpecifier(_)
                );
