...
fn missing_parameters(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Missing parameters.")
        .with_help("Add parameters for parsing numbers, e.g., `parseInt('10', 10)`.")
        .with_label(span)
}

fn missing_radix(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Missing radix parameter.")
        .with_help("Add radix parameter `10` for parsing decimal numbers, or specify the appropriate radix for other number formats.")
        .with_label(span)
}

fn invalid_radix(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Invalid radix parameter, must be an integer between 2 and 36.")
