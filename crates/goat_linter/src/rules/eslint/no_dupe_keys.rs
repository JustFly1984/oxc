...
fn no_dupe_keys_diagnostic(first: Span, second: Span, key: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("Duplicate key '{key}'"))
...
fn is_proto_setter_property(prop: &goat_ast::ast::ObjectProperty<'_>, name: &str) -> bool {
    name == "__proto__"
        && prop.kind == PropertyKind::Init
        && !prop.computed
        && !prop.shorthand
