...
use goat_span::Span;
use goat_syntax::operator::{AssignmentOperator, BinaryOperator, LogicalOperator, UnaryOperator};
...
fn constant_short_circuit(lhs_name: &str, expr_name: &str, span: Span) -> GoatDiagnostic {
...
fn constant_binary_operand(left_or_right: &str, operator: &str, span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Unexpected constant binary expression")
        .with_help(format!(
...
fn constant_always_new(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Unexpected comparison to newly constructed object")
        .with_help("These two values can never be equal")
        .with_label(span)
}

fn constant_both_always_new(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Unexpected comparison of two newly constructed objects")
        .with_help("These two values can never be equal")
        .with_label(span)
}
