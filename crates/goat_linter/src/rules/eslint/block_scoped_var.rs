...
use goat_syntax::{scope::ScopeId, symbol::SymbolId};

fn redeclaration_diagnostic(decl_span: Span, redeclare_span: Span, name: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("'{name}' is used outside of binding context."))
...
fn use_outside_scope_diagnostic(decl_span: Span, used_span: Span, name: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("'{name}' is used outside of binding context."))
        .with_help(format!("Variable '{name}' is used outside its declaration block. Declare it outside the block or use 'let'/'const'."))
        .with_labels([
