...
use goat_span::{GetSpan, Span, best_match};
...
fn not_string(help: Option<&'static str>, span: Span) -> GoatDiagnostic {
    let mut d =
        GoatDiagnostic::warn("`typeof` comparisons should be to string literals.").with_label(span);
    if let Some(x) = help {
        d = d.with_help(x);
    }
    d
...
fn invalid_value(help: Option<String>, span: Span) -> GoatDiagnostic {
    let mut d = GoatDiagnostic::warn("Invalid `typeof` comparison value.").with_label(span);
    if let Some(x) = help {
        d = d.with_help(x);
    }
    d
