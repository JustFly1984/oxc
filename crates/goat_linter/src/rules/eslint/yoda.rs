...
use goat_ecmascript::{ToBigInt, WithoutGlobalReferenceInformation};
...
fn yoda_diagnostic(span: Span, never: bool, operator: &str) -> GoatDiagnostic {
    let expected_side = if never { "right" } else { "left" };
    GoatDiagnostic::warn("Require or disallow \"Yoda\" conditions")
