...
fn unexpected_unicode_bom_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Unexpected Unicode BOM (Byte Order Mark)")
        .with_help("File must not begin with the Unicode BOM")
        .with_label(span)
}

fn expected_unicode_bom_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Expected Unicode BOM (Byte Order Mark)")
