...
fn expected_all_properties_shorthanded(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Expected shorthand for all properties.").with_label(span)
}
...
fn expected_property_shorthand(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Expected property shorthand.").with_label(span)
}

fn expected_property_longform(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Expected longform property syntax.").with_label(span)
}

fn expected_method_shorthand(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Expected method shorthand.").with_label(span)
}

fn expected_method_longform(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Expected longform method syntax.").with_label(span)
}

fn unexpected_mix(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Unexpected mix of shorthand and non-shorthand properties.")
        .with_label(span)
}
...
impl<'a> Visit<'a> for ArrowFunctionLexicalIdentifierVisitor<'a, '_> {
    fn visit_function(
        &mut self,
        _it: &goat_ast::ast::Function<'a>,
        _flags: goat_semantic::ScopeFlags,
    ) {
    }
    ...
    fn visit_this_expression(&mut self, _it: &goat_ast::ast::ThisExpression) {
        self.has_lexical_identifier = true;
    }

    fn visit_super(&mut self, _it: &goat_ast::ast::Super) {
        self.has_lexical_identifier = true;
    }

    fn visit_meta_property(&mut self, it: &goat_ast::ast::MetaProperty<'a>) {
        if it.meta.name == "new" && it.property.name == "target" {
            self.has_lexical_identifier = true;
        }
    }

    fn visit_identifier_reference(&mut self, it: &goat_ast::ast::IdentifierReference<'a>) {
