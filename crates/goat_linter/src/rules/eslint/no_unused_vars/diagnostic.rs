...
pub fn used_ignored<R>(symbol: &Symbol<'_, '_>, pat: &IgnorePattern<R>) -> GoatDiagnostic
where
    R: fmt::Display,
{
    ...
    GoatDiagnostic::warn(format!("{pronoun_singular} '{name}' is marked as ignored but is used."))
    ...
    R: fmt::Display,
{
    ...
    GoatDiagnostic::warn(if only_used_as_type {
    ...
    R: fmt::Display,
{
    ...
    GoatDiagnostic::warn(format!("{pronoun} '{name}' is assigned a value but never used.{suffix}"))
        .with_labels([
...
pub fn param<R>(symbol: &Symbol<'_, '_>, pat: &IgnorePattern<R>) -> GoatDiagnostic
where
    R: fmt::Display,
{
    let name = symbol.name();
    ...
    GoatDiagnostic::warn(format!("Parameter '{name}' is declared but never used.{suffix}"))
        .with_label(symbol.span().label(format!("'{name}' is declared here")))
        .with_help("Consider removing this parameter.")
}

/// Identifier 'x' imported but never used.
pub fn imported(symbol: &Symbol<'_, '_>) -> GoatDiagnostic {
    let (pronoun, _) = pronoun_for_symbol(symbol.flags());
    let name = symbol.name();

    GoatDiagnostic::warn(format!("{pronoun} '{name}' is imported but never used."))
