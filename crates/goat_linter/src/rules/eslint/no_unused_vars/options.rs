...
fn invalid_option_mismatch_error<E, A>(option_name: &str, expected: E, actual: A) -> GoatDiagnostic
where
...
fn invalid_option_error<M: Into<Cow<'static, str>>>(
    ...
    GoatDiagnostic::error(format!("Invalid '{option_name}' option for no-unused-vars: {message}"))
}

impl TryFrom<&String> for VarsOption {
    type Error = GoatDiagnostic;
...
fn parse_unicode_rule(value: Option<&Value>, name: &str) -> IgnorePattern<Regex> {
    IgnorePattern::try_from(value.and_then(Value::as_str))
        .map_err(|err| {
            GoatDiagnostic::error(format!("Invalid '{name}' option for no-unused-vars: {err}"))
        })
        .unwrap()
}
...
impl TryFrom<Value> for NoUnusedVarsOptions {
    ...
    fn try_from(value: Value) -> Result<Self, Self::Error> {
        ...
        match config {
            ...
            _ => Err(GoatDiagnostic::error(format!(
...
mod tests {
    ...
    fn test_invalid() {
        ...
        for options in invalid_options {
            let result: Result<NoUnusedVarsOptions, GoatDiagnostic> = options.try_into();
