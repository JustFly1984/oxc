...
fn guess_missing_return_hint(
    ...
    match last_statement {
        Statement::SwitchStatement(stmt) => {
            let has_default = stmt.cases.iter().any(goat_ast::ast::SwitchCase::is_default_case);
...
fn expect_return(
    ...
    let mut diagnostic = GoatDiagnostic::warn(message).with_help(help).with_label(span);
    if allow_implicit {
...
fn expect_no_return(method_name: &str, call_span: Span, return_span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("Unexpected return value in callback for {method_name:?}"))
        .with_help(format!(
...
fn expect_void_return(method_name: &str, call_span: Span, return_span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("Unexpected return value in callback for {method_name:?}"))
