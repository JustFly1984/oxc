use goat_allocator::Vec as AllocatorVec;
use goat_ast::ast::{
...
use goat_semantic::{NodeId, ScopeFlags, Semantic};
...
pub fn check_function_body(function_node_id: NodeId, semantic: &Semantic) -> StatementReturnStatus {
    ...
    while let Some((block_id, incoming_exit)) = worklist.pop() {
        ...
        for edge in cfg.graph().edges_directed(block_id, Direction::Outgoing) {
            ...
            has_successor = true;
            let propagated_exit =
                if matches!(edge_kind, EdgeType::Error(goat_cfg::ErrorEdgeKind::Explicit)) {
                    incoming_exit
                } else {
                    state_after_block
                };
