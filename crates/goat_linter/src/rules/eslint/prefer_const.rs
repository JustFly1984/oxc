use goat_ast::ast::{AssignmentTarget, AssignmentTargetMaybeDefault, AssignmentTargetProperty};
use goat_ast::{AstKind, ast::VariableDeclarationKind};
use goat_diagnostics::GoatDiagnostic;
use goat_macros::declare_goat_lint;
use goat_semantic::SymbolId;
...
fn prefer_const_diagnostic(name: &str, span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("`{name}` is never reassigned."))
...
impl PreferConst {
    ...
    fn fix_let_to_const<'a>(
        fixer: RuleFixer<'_, 'a>,
        decl: &goat_ast::ast::VariableDeclaration<'a>,
    ...
    fn can_identifier_be_const(
        &self,
        symbol_id: SymbolId,
        symbol_table: &goat_semantic::Scoping,
        ctx: &LintContext<'_>,
    ) -> bool {
        ...
        if matches!(ctx.nodes().kind(decl_node), goat_ast::AstKind::FormalParameter(_)) {
            return false;
        }
    ...
    fn can_all_destructuring_identifiers_be_const(
        &self,
        array_target: &goat_ast::ast::ArrayAssignmentTarget,
        symbol_table: &goat_semantic::Scoping,
        ctx: &LintContext<'_>,
    ) -> bool {
    ...
    fn can_all_destructuring_identifiers_be_const_obj(
        &self,
        obj_target: &goat_ast::ast::ObjectAssignmentTarget,
        symbol_table: &goat_semantic::Scoping,
        ctx: &LintContext<'_>,
    ) -> bool {
        use goat_ast::ast::AssignmentTargetProperty;
...
fn test() {
    ...
    let fix = vec![
        ...
        (
            ...
            // This means the user would have to run goatlint --fix multiple times to get all fixes.
