use goat_ast::{AstKind, ast::Expression, match_member_expression};
use goat_diagnostics::GoatDiagnostic;
use goat_macros::declare_goat_lint;
use goat_span::{GetSpan, Span};
use goat_syntax::precedence::{GetPrecedence, Precedence};
...
fn prefer_exponentian_operator_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Prefer `**` over `Math.pow`.")
...
fn needs_parens_for_parent(node: &AstNode, ctx: &LintContext) -> bool {
    ...
    match parent.kind() {
        AstKind::BinaryExpression(bin_expr) => {
            if bin_expr.operator == goat_ast::ast::BinaryOperator::Exponential {
                let AstKind::CallExpression(call_expr) = node.kind() else {
                    return true;
                };
