...
use goat_span::{ContentEq, GetSpan, Span};
...
fn id_length_is_too_short_diagnostic(span: Span, config_min: u64) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("Identifier name is too short (< {config_min}).")).with_label(span)
}

fn id_length_is_too_long_diagnostic(span: Span, config_max: u64) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("Identifier name is too long (> {config_max}).")).with_label(span)
}
