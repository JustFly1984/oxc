...
use goat_regular_expression::{ConstructorParser, Options};
use goat_span::Span;
...
// Use the same prefix with `goat_regular_expression` crate
fn duplicated_flag_diagnostic(span: Span, flag: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn("Invalid regular expression: Duplicated flag")
        .with_help(
...
fn unknown_flag_diagnostic(span: Span, flag: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn("Invalid regular expression: Unknown flag")
...
fn invalid_unicode_flags_diagnostic(span: Span, is_u_specified: bool) -> GoatDiagnostic {
    GoatDiagnostic::warn("Invalid regular expression: `u` and `v` flags should be used alone")
...
impl Rule for NoInvalidRegexp {
    ...
    fn run<'a>(&self, node: &AstNode<'a>, ctx: &LintContext<'a>) {
        ...
        // `goat_regular_expression` crate has a ability to validate flags.
