...
use goat_semantic::{IsGlobalReference, ScopeFlags};
use goat_span::{
    GetSpan, Span,
...
fn preserve_caught_error_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("There is no cause error attached to this new thrown error.")
        .with_help(
...
fn missing_catch_parameter_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn(
...
fn is_catch_parameter(expr: &Expression, catch_param: &BindingPattern, ctx: &LintContext) -> bool {
    let goat_ast::ast::BindingPattern::BindingIdentifier(binding) = &catch_param else {
        return false;
    };
