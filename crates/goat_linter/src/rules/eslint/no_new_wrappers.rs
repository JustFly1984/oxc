...
fn no_new_wrappers_diagnostic(builtin_name: &str, new_span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("Do not use `{builtin_name}` as a constructor"))
        .with_help("Remove the `new` operator.")
        .with_label(new_span)
}

fn not_a_constructor_diagnostic(builtin_name: &str, new_span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("`{builtin_name}` is not a constructor"))
        .with_help("Remove the `new` operator.")
        .with_label(new_span)
}
