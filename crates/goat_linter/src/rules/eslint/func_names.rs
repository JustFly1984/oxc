...
use goat_span::{GetSpan, Ident, Span};
use goat_syntax::{identifier::is_identifier_name, keyword::is_reserved_keyword_or_global_object};
...
fn named_diagnostic(function_name: &str, span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("Unexpected named {function_name}."))
        .with_label(span)
        .with_help("Remove the name on this function expression.")
}

fn unnamed_diagnostic(inferred_name_or_description: &str, span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("Unexpected unnamed {inferred_name_or_description}."))
        .with_label(span)
