...
use goat_semantic::{AstNode, ScopeId};
use goat_span::Span;
...
fn no_obj_calls_diagnostic(obj_name: &str, span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("`{obj_name}` is not a function and cannot be called"))
...
fn resolve_global_binding<'a, 'b: 'a>(
    ident: &'a goat_allocator::Box<'a, IdentifierReference<'a>>,
    scope_id: ScopeId,
    ctx: &LintContext<'a>,
) -> Option<&'a str> {
