...
fn must_use_const_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Number constants declarations must use 'const'.")
        .with_help("Use 'const' instead of 'let' or 'var' to declare number constants to make their immutability explicit.")
        .with_label(span)
}

fn no_magic_number_diagnostic(span: Span, raw: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("No magic number: {raw}"))
