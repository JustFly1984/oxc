...
fn prefer_function_type_diagnostic(suggestion: &str, span: Span) -> GoatDiagnostic {
    // FIXME: use imperative message phrasing
    GoatDiagnostic::warn("Enforce using function types instead of interfaces with call signatures.")
    ...
    fn run<'a>(&self, node: &AstNode<'a>, ctx: &LintContext<'a>) {
        match node.kind() {
            AstKind::TSInterfaceDeclaration(decl) => {
                let body: &goat_allocator::Vec<'_, TSSignature<'_>> = &decl.body.body;
