...
fn comment(ts_comment_name: &str, span: Span) -> GoatDiagnostic {
...
fn ignore_instead_of_expect_error(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Use \"@ts-expect-error\" instead of @ts-ignore, as \"@ts-ignore\" will do nothing if the following line is error-free.")
        .with_help("Replace \"@ts-ignore\" with \"@ts-expect-error\".")
        .with_label(span)
}

fn comment_requires_description(ts_comment_name: &str, min_len: u64, span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!(
