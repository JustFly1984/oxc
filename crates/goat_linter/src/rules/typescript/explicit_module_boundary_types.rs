...
use goat_allocator::{Address, UnstableAddress};
...
use goat_span::{CompactStr, GetSpan, Ident, Span};
...
fn func_missing_return_type(fn_span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Missing return type on function")
        .with_help("Define an explicit return type for the function.")
        .with_label(fn_span)
}

fn func_missing_argument_type(param_span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Missing argument type on function")
        .with_help("Define an explicit argument type for each argument.")
        .with_label(param_span)
}

fn func_argument_is_explicitly_any(param_span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Argument is explicitly typed as `any`")
        .with_help(
...
impl<'a> Visit<'a> for ExplicitTypesChecker<'a, '_> {
    ...
    fn visit_statements(&mut self, it: &goat_allocator::Vec<'a, Statement<'a>>) {
        for stmt in it {
            match stmt {
