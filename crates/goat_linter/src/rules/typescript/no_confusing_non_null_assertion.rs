...
use goat_span::Span;
use goat_syntax::operator::{AssignmentOperator, BinaryOperator};
...
fn not_need_no_confusing_non_null_assertion_diagnostic(op_str: &str, span: Span) -> GoatDiagnostic {
...
fn wrap_up_no_confusing_non_null_assertion_diagnostic(op_str: &str, span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!(
        "Confusing combinations of non-null assertion and equal test like \"a! {op_str} b\", which looks very similar to not equal \"a !{op_str} b\"."
    ))
...
fn confusing_non_null_assignment_assertion_diagnostic(op_str: &str, span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!(
