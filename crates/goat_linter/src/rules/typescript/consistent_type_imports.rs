...
use goat_semantic::{Reference, SymbolId};
...
fn no_import_type_annotations_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("`import()` type annotations are forbidden.")
...
fn avoid_import_type_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Use an `import` instead of an `import type`.")
...
fn type_over_value_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("All imports in the declaration are only used as types. Use `import type`.")
...
fn some_imports_are_only_types_diagnostic(span: Span, type_imports: &str) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("Imports {type_imports} are only used as type.")).with_label(span)
}
