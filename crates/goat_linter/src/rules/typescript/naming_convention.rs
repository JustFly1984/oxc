...
use goat_ast::ast::{BindingPattern, VariableDeclarationKind};
...
fn naming_convention_diagnostic(
    ...
    GoatDiagnostic::warn(format!("{kind} `{name}` does not match the expected naming convention."))
    ...
    fn run<'a>(&self, node: &AstNode<'a>, ctx: &LintContext<'a>) {
        ...
        match node.kind() {
            ...
            AstKind::TSEnumMember(member) => {
                if let goat_ast::ast::TSEnumMemberName::Identifier(id) = &member.id {
                    let name = id.name.as_str();
