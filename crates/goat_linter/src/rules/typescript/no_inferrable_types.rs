...
fn no_inferrable_types_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Type can be trivially inferred from the initializer")
...
impl NoInferrableTypes {
    fn check_formal_parameters<'a>(
        &self,
        params: &goat_allocator::Vec<'a, FormalParameter<'a>>,
        ctx: &LintContext<'a>,
    ) {
