...
fn use_angle_bracket_diagnostic(cast: &str, span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("Use `<{cast}>` instead of `as {cast}`."))
        .with_help(format!("Replace `as {cast}` with `<{cast}>`. For example, change `value as {cast}` to `<{cast}>value`."))
        .with_label(span)
}

fn use_as_diagnostic(cast: &str, span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn(format!("Use `as {cast}` instead of `<{cast}>`.")).with_label(span)
}

fn never_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Do not use any type assertions.")
...
fn unexpected_object_type_assertion_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Always prefer `const x: T = { ... }`.")
...
fn unexpected_array_type_assertion_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Always prefer `const x: T[] = [ ... ]`.")
