...
fn empty_class_diagnostic(span: Span, has_decorators: bool) -> GoatDiagnostic {
    let help = if has_decorators {
        r#"Set "allowWithDecorator": true in your config to allow empty decorated classes"#
    } else {
        "Delete this class"
    };
    GoatDiagnostic::warn("Unexpected empty class.").with_label(span).with_help(help)
}

fn only_static_no_extraneous_class_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Unexpected class with only static properties.")
        .with_label(span)
        .with_help("Try using standalone functions instead of static methods")
}

fn only_constructor_no_extraneous_class_diagnostic(span: Span) -> GoatDiagnostic {
    GoatDiagnostic::warn("Unexpected class with only a constructor.")
        .with_label(span)
