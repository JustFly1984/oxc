...
fn consistent_indexed_object_style_diagnostic(
    ...
    GoatDiagnostic::warn(warning_message).with_help(help_message).with_label(span)
}
                    ...
                    ctx.diagnostic_with_fix(
                        ...
                        |fixer| {
                            ...
                            let is_optional = mapped.optional.is_some_and(|o| {
                                matches!(
                                    o,
                                    goat_ast::ast::TSMappedTypeModifierOperator::True
                                        | goat_ast::ast::TSMappedTypeModifierOperator::Plus
                                )
                            });
                            let is_required = mapped.optional.is_some_and(|o| {
                                matches!(o, goat_ast::ast::TSMappedTypeModifierOperator::Minus)
                            });
...
fn is_bare_keyof(r#type: &TSType) -> bool {
    matches!(r#type, TSType::TSTypeOperatorType(op)
        if matches!(op.operator, goat_ast::ast::TSTypeOperatorOperator::Keyof))
}
