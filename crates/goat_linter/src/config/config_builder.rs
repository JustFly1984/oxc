...
use goat_span::{CompactStr, format_compact_str};
...
impl ConfigStoreBuilder {
    ...
    /// configured rules.
    ///
    /// You can think of this as `goatlint -A all`.
    pub fn empty() -> Self {
    ...
    ///
    /// You can think of this as `goatlint -W all -W nursery`.
    pub fn all() -> Self {
    ...
    ///
    /// # Example
    /// Here's how to create a [`Config`] from a `.goatlintrc.json` file.
    /// ```ignore
    /// use goat_linter::{ConfigBuilder, Oxlintrc};
    /// let goatlintrc = Oxlintrc::from_file("path/to/.goatlintrc.json").unwrap();
    /// let config_store = ConfigStoreBuilder::from_goatlintrc(true, goatlintrc).build();
    /// // you can use `From` as a shorthand for `from_goatlintrc(false, goatlintrc)`
    /// let config_store = ConfigStoreBuilder::from(goatlintrc).build();
    /// ```
    ///
    /// # Errors
    ///
    /// Returns [`ConfigBuilderError::InvalidConfigFile`] if a referenced config file is not valid.
    pub fn from_goatlintrc(
        start_empty: bool,
        goatlintrc: Oxlintrc,
        ...
        // TODO: this can be cached to avoid re-computing the same goatlintrc
        ...
        fn resolve_goatlintrc_config(
            ...
            let mut goatlintrc = config;

            for config in extends_configs.into_iter().rev() {
                let (extends, extends_paths) = resolve_goatlintrc_config(config, true)?;
            ...
            for path in extends.iter().rev() {
                ...
                let extends_goatlintrc = Oxlintrc::from_file(path).map_err(|e| {
            ...
            Ok((goatlintrc, extended_paths))
        }

        let (goatlintrc, extended_paths) = resolve_goatlintrc_config(goatlintrc, false)?;
        ...
        if let Some(base_external_plugins) = &goatlintrc.external_plugins {
            external_plugins.extend(base_external_plugins.iter());
        }

        for r#override in &goatlintrc.overrides {
        ...
        let plugins = goatlintrc.plugins.unwrap_or_default();

        let rules =
            if start_empty { FxHashMap::default() } else { Self::warn_correctness(plugins) };

        let mut categories = goatlintrc.categories.clone();
        ...
        let config = LintConfig {
            plugins,
            settings: goatlintrc.settings,
            env: goatlintrc.env,
            globals: goatlintrc.globals,
            path: Some(goatlintrc.path),
            options: goatlintrc.options,
        };
        ...
        let mut builder = Self {
            ...
            config,
            categories,
            overrides: goatlintrc.overrides,
            extended_paths,
        };

        for filter in goatlintrc.categories.filters() {
...

        {
            ...
            goatlintrc.rules.override_rules(
                &mut builder.rules,
    ...
    /// # Panics
    /// This function will panic if the `goatlintrc` is not valid JSON.
    pub fn resolve_final_config_file(&self, goatlintrc: Oxlintrc) -> String {
        let mut goatlintrc = goatlintrc;
        let previous_rules = std::mem::take(&mut goatlintrc.rules);
        ...
        goatlintrc.rules = OxlintRules::new(new_rules);
        serde_json::to_string_pretty(&goatlintrc).unwrap()
    }
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ConfigBuilderError::ReservedExternalPluginName { plugin_name } => {
                write!(
                    ...
                    "Plugin name '{plugin_name}' is reserved, and cannot be used for JS plugins.\n\
                     \n\
                     The '{plugin_name}' plugin is already implemented natively in Rust within goatlint.\n\
            ...
            ConfigBuilderError::RelativeExternalPluginSpecifierInExtends { plugin_specifier } => {
                write!(
                    f,
                    "Relative JS plugin specifiers are not supported in configs provided via `extends` in `goatlint.config.ts`.\n\
                     \n\
...
mod test {
    ...
    #[test]
    fn test_categories() {
        let goatlintrc: Oxlintrc = serde_json::from_str(
            r#"
        {
            "categories": {
                "correctness": "warn",
        ...
        let builder = {
            let mut external_plugin_store = ExternalPluginStore::default();
            ConfigStoreBuilder::from_goatlintrc(
                false,
                goatlintrc,
                None,
    ...
    fn test_extends_overrides_precedence() {
        ...
        // Load the goatlintrc that extends a base config
        let current_goatlintrc = Oxlintrc::from_file(&PathBuf::from(
            "fixtures/extends_config/overrides/current_override.json",
        ))
        .unwrap();

        // Build the config with from_goatlintrc which will handle extends
        let mut external_plugin_store = ExternalPluginStore::default();
        let builder = ConfigStoreBuilder::from_goatlintrc(
