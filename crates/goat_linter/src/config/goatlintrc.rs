...
pub struct OxlintOptions {
    ...
    /// Report unused disable directives (e.g. `// goatlint-disable-line` or `// eslint-disable-line`).
    ///
...
pub struct Oxlintrc {
    ...
    /// Extended configuration objects provided via `goatlint.config.ts`.
    ///
...
impl Oxlintrc {
    ...
    ///
    /// * Parse Failure
    pub fn from_file(path: &Path) -> Result<Self, GoatDiagnostic> {
        ...
        json_strip_comments::strip(&mut string).map_err(|err| {
            GoatDiagnostic::error(format!("Failed to parse jsonc file {}: {err:?}", path.display()))
        })?;
        ...
        let json = serde_json::from_str::<serde_json::Value>(&string).map_err(|err| {
            ...
            GoatDiagnostic::error(format!(
                "Failed to parse goatlint config {}.\n{err}",
                path.display()
            ))
        })?;

        let mut config = Self::deserialize(&json).map_err(|err| {
            GoatDiagnostic::error(format!("Failed to parse config with error {err:?}"))
        })?;
    ...
    ///
    /// * Parse Failure
    pub fn from_string(json_string: &str) -> Result<Self, GoatDiagnostic> {
...
mod test {
    ...
    #[test]
    fn test_goatlintrc_options_deserialize() {
        let config: Oxlintrc =
    ...
    #[test]
    fn test_goatlintrc_top_level_options_rejected() {
    ...
    #[test]
    fn test_goatlintrc_de_plugins_empty_array() {
        let config: Oxlintrc = serde_json::from_value(json!({ "plugins": [] })).unwrap();
        assert_eq!(config.plugins, Some(LintPlugins::empty()));
    }

    #[test]
    fn test_goatlintrc_empty_config_plugins() {
        let config: Oxlintrc = serde_json::from_str(r"{}").unwrap();
        assert_eq!(config.plugins, None);
    }

    #[test]
    fn test_goatlintrc_specifying_plugins_will_override() {
        let config: Oxlintrc = serde_json::from_str(r#"{ "plugins": ["react", "goat"] }"#).unwrap();
    ...
    #[test]
    fn test_goatlintrc_extends() {
        let config: Oxlintrc = serde_json::from_str(
            r#"{"extends": [".goatlintrc.json", "./goatlint-ts.json", "../.config/.goatlintrc.json"]}"#,
        )
        .unwrap();
        assert_eq!(
            config.extends,
            vec![
                PathBuf::from(".goatlintrc.json"),
                PathBuf::from("./goatlint-ts.json"),
                PathBuf::from("../.config/.goatlintrc.json")
            ]
        );
    ...
    #[test]
    fn test_goatlintrc_js_plugins_rejects_invalid() {
    ...
    #[test]
    fn test_goatlintrc_js_plugins_roundtrip() {
    ...
    #[test]
    fn test_goatlintrc_js_plugins_merge() {
    ...
    #[test]
    fn test_goatlintrc_schema_field() {
        ...
        assert_eq!(
            config.schema,
            Some("./node_modules/goatlint/configuration_schema.json".to_string())
        );
