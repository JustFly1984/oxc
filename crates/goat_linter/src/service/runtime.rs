...
struct ProcessedModule<'alloc_pool> {
    ...
    section_module_records: SmallVec<[Result<ResolvedModuleRecord, Vec<GoatDiagnostic>>; 1]>,
...
struct SectionContent<'a> {
    ...
    /// `ProcessedModule.section_module_records` would be `Err(Vec<GoatDiagnostic>)`.
...
struct ModuleToLint<'alloc_pool> {
    path: Arc<OsStr>,
    section_module_records: SmallVec<[Result<Arc<ModuleRecord>, Vec<GoatDiagnostic>>; 1]>,
...
impl Runtime {
    ...
    fn get_source_type_and_text<'a>(
        ...
        let file_result = file_system.read_to_arena_str(path, allocator).map_err(|e| {
            Error::new(GoatDiagnostic::error(format!(
    ...
    fn process_path_to_module<'a>(
        ...
        if paths.contains(path) {
            let mut records =
                SmallVec::<[Result<ResolvedModuleRecord, Vec<GoatDiagnostic>>; 1]>::new();
    ...
    fn process_source<'a>(
    ...
    ) -> SmallVec<[Result<ResolvedModuleRecord, Vec<GoatDiagnostic>>; 1]> {
        ...
        let mut section_module_records = SmallVec::<
            [Result<ResolvedModuleRecord, Vec<GoatDiagnostic>>; 1],
        ...
        for section_source in section_sources {
            match self.process_source_section(
                ...
                Err(err) => {
                    let err: Vec<GoatDiagnostic> = err
                        .into_iter()
                        .map(|mut diagnostic| {
    ...
    fn process_source_section<'a>(
        ...
        source_type: SourceType,
        check_syntax_errors: bool,
    ) -> Result<(ResolvedModuleRecord, Semantic<'a>, ArenaBox<'a, [Token]>), Vec<GoatDiagnostic>>
    {
