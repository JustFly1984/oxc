...
pub struct ParsedExpectFnCall<'a> {
    ...
    pub args: &'a goat_allocator::Vec<'a, Argument<'a>>,
    ...
    pub expect_arguments: Option<&'a goat_allocator::Vec<'a, Argument<'a>>>,
    /// the arguments passed to the matcher function
    /// In `expect(1).toBe(2)`, it will be `[2]
    /// In `expect(1)`, it will be `None`
    pub matcher_arguments: Option<&'a goat_allocator::Vec<'a, Argument<'a>>>,
}
