...
fn react_perf_inline_diagnostic(message: &'static str, attr_span: Span) -> GoatDiagnostic {
...
fn react_perf_reference_diagnostic(
    ...
    let mut diagnostic = GoatDiagnostic::warn(message)
        .with_help(r"simplify props or memoize props in the parent component (https://react.dev/reference/react/memo#my-component-rerenders-when-a-prop-is-an-object-or-array).")
        .with_label(
...
pub trait ReactPerfRule: Sized + Default + fmt::Debug {
    ...
    /// report the [`GoatDiagnostic`] and return `true`.
    ///
    /// [`GoatDiagnostic`]: goat_diagnostics::GoatDiagnostic
