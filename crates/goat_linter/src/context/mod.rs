...
use goat_ast::ast::IdentifierReference;
use goat_cfg::ControlFlowGraph;
use goat_diagnostics::{GoatDiagnostic, Severity};
...
impl<'a> LintContext<'a> {
    ...
    #[inline]
    pub fn diagnostic(&self, diagnostic: GoatDiagnostic) {
        self.add_diagnostic(
    ...
    #[inline]
    pub fn diagnostic_with_fix<C, F>(&self, diagnostic: GoatDiagnostic, fix: F)
    where
        C: Into<RuleFix>,
    ...
    /// [closure]: <https://doc.rust-lang.org/book/ch13-01-closures.html>
    #[inline]
    pub fn diagnostic_with_suggestion<C, F>(&self, diagnostic: GoatDiagnostic, fix: F)
    where
        C: Into<RuleFix>,
        F: FnOnce(RuleFixer<'_, 'a>) -> C,
    {
    ...
    /// [closure]: <https://doc.rust-lang.org/book/ch13-01-closures.html>
    #[inline]
    pub fn diagnostic_with_dangerous_suggestion<C, F>(&self, diagnostic: GoatDiagnostic, fix: F)
    where
        C: Into<RuleFix>,
        F: FnOnce(RuleFixer<'_, 'a>) -> C,
    {
    ...
    ///
    #[inline]
    pub fn diagnostic_with_dangerous_fix<C, F>(&self, diagnostic: GoatDiagnostic, fix: F)
    where
        C: Into<RuleFix>,
        F: FnOnce(RuleFixer<'_, 'a>) -> C,
    {
    ...
    pub fn diagnostic_with_suggestions<I>(&self, diagnostic: GoatDiagnostic, suggestions: I)
    where
        ...
        let fixes_result: Vec<Fix> = suggestions
            ...
            .filter_map(|rule_fix| {
                ...
                debug_assert!(
                    ...
                    "Rule `{}` does not support this fix kind. Did you forget to update fix capabilities in declare_goat_lint?.\n\tSupported fix kinds: {:?}\n\tAttempted fix kind: {:?}",
        ...
        F: FnOnce(RuleFixer<'_, 'a>) -> C,
    {
        ...
        debug_assert!(
            ...
            "Rule `{}` does not support safe fixes. Did you forget to update fix capabilities in declare_goat_lint?.\n\tSupported fix kinds: {:?}\n\tAttempted fix kind: {:?}",
            self.current_rule_name,
            FixKind::from(self.current_rule_fix_capabilities),
            rule_fix.kind()
        );
