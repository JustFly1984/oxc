...
use goat_diagnostics::{DiagnosticSender, DiagnosticService, GoatDiagnostic, Severity};
...
pub struct TsGoLintState {
    ...
    cwd: PathBuf,
    /// The configuration store for `tsgolint` (used to resolve configurations outside of `goat_linter`)
    config_store: ConfigStore,
    /// If `goatlint` will output the diagnostics or not.
                            ...
                            match tsgolint_diagnostic {
                                ...
                                TsGoLintDiagnostic::Internal(e) => {
                                    let span = e
                                        ...
                                        .flatten()
                                        .unwrap_or_default();
                                    let mut diagnostic: GoatDiagnostic = e.into();
...
impl From<TsGoLintDiagnostic> for GoatDiagnostic {
...
impl From<TsGoLintRuleDiagnostic> for GoatDiagnostic {
    fn from(val: TsGoLintRuleDiagnostic) -> Self {
        let mut d = GoatDiagnostic::warn(val.message.description)
...
impl From<TsGoLintInternalDiagnostic> for GoatDiagnostic {
    fn from(val: TsGoLintInternalDiagnostic) -> Self {
        let mut d = GoatDiagnostic::error(val.message.description)
...
impl DiagnosticHandler {
    ...
    fn handle_internal_diagnostic(&mut self, e: TsGoLintInternalDiagnostic) {
        ...
        let oxc_diagnostic: GoatDiagnostic = e.into();
...
pub fn try_find_tsgolint_executable(cwd: &Path) -> Result<PathBuf, String> {
    ...
    Err("Failed to find tsgolint executable. You may need to add the `goatlint-tsgolint` package to your project?".to_string())
}

#[cfg(test)]
mod test {
    use goat_diagnostics::{LabeledSpan, OxcCode, Severity};
    use goat_span::Span;
