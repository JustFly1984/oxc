...
impl Message {
    ...
    pub fn new(error: GoatDiagnostic, fixes: PossibleFixes) -> Self {
        let span = error
            .labels
            .as_ref()
...
impl From<Message> for GoatDiagnostic {
    #[inline]
...
pub struct Fixer<'a> {
    ...
    // The behavior is oriented by `goatlint` where only one PossibleFixes is applied.
    fix_index: u8,
...
mod test {
    ...
    fn insert_at_end() -> GoatDiagnostic {
        GoatDiagnostic::warn("End")
    }

    fn insert_at_start() -> GoatDiagnostic {
        GoatDiagnostic::warn("Start")
    }

    fn insert_at_middle() -> GoatDiagnostic {
        GoatDiagnostic::warn("Multiply")
    }

    fn replace_id() -> GoatDiagnostic {
        GoatDiagnostic::warn("foo")
    }

    fn replace_var() -> GoatDiagnostic {
        GoatDiagnostic::warn("let")
    }

    fn replace_num() -> GoatDiagnostic {
        GoatDiagnostic::warn("5")
    }

    fn remove_start() -> GoatDiagnostic {
        GoatDiagnostic::warn("removestart")
    }

    fn remove_middle(span: Span) -> GoatDiagnostic {
        GoatDiagnostic::warn("removemiddle").with_label(span)
    }

    fn remove_end() -> GoatDiagnostic {
        GoatDiagnostic::warn("removeend")
    }

    fn reverse_range() -> GoatDiagnostic {
        GoatDiagnostic::warn("reversed range")
    }

    fn no_fix(span: Span) -> GoatDiagnostic {
        GoatDiagnostic::warn("nofix").with_label(span)
    }

    fn no_fix_1(span: Span) -> GoatDiagnostic {
        GoatDiagnostic::warn("nofix1").with_label(span)
    }

    fn no_fix_2(span: Span) -> GoatDiagnostic {
        GoatDiagnostic::warn("nofix2").with_label(span)
    }
    ...
    fn create_message(error: GoatDiagnostic, fix: PossibleFixes) -> Message {
    ...
    fn debug_assert_catches_invalid_fix() {
        ...
        let message =
            create_message(GoatDiagnostic::warn("Invalid fix test"), PossibleFixes::Single(fix));
    ...
    fn valid_fix_passes_debug_assertion() {
        ...
        let message =
            create_message(GoatDiagnostic::warn("Valid fix test"), PossibleFixes::Single(fix));
