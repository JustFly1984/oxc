...
#[doc(hidden)]
pub mod __internal {
    // Used by `format_compact_str!` macro defined in `goat_str`
    pub use compact_str::format_compact;
    // Used by `format_str!` and `format_ident!` macros defined in `goat_str`
    pub use goat_allocator::StringBuilder as ArenaStringBuilder;
}

// Additional trait implementations for types re-exported from goat_str
use std::ops::Index;
