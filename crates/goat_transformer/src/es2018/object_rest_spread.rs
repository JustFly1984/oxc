...
use goat_allocator::{Box as ArenaBox, GetAddress, TakeIn, Vec as ArenaVec};
use goat_ast::{NONE, ast::*};
use goat_diagnostics::GoatDiagnostic;
use goat_ecmascript::{BoundNames, ToJsString, WithoutGlobalReferenceInformation};
use goat_semantic::{ScopeFlags, ScopeId, SymbolFlags};
use goat_span::{GetSpan, SPAN};
use goat_traverse::{Ancestor, MaybeBoundIdentifier, Traverse};
...
impl<'a> ObjectRestSpread<'a> {
    pub fn new(options: ObjectRestSpreadOptions, state: &mut TransformState<'a>) -> Self {
        if options.loose {
            state.error(GoatDiagnostic::error(
