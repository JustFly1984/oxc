use goat_allocator::{TakeIn, Vec as ArenaVec};
use goat_ast::ast::*;
use goat_diagnostics::GoatDiagnostic;
use goat_semantic::{Reference, SymbolFlags};
use goat_span::{GetSpan, SPAN, Span, Str};
use goat_syntax::{
...
impl<'a> Traverse<'a, TransformState<'a>> for TypeScriptAnnotations<'a> {
        ...
        if let Some(expr) = target.get_expression_mut() {
            match expr.get_inner_expression_mut() {
                ...
                _ => {
                    ...
                    ctx.state.error(GoatDiagnostic::error("Cannot strip out typescript syntax if SimpleAssignmentTarget is not an IdentifierReference or MemberExpression"));
                }
            }
        }
    }
    ...
    fn enter_formal_parameter_rest(
        ...
        _ctx: &mut goat_traverse::TraverseCtx<'a, TransformState<'a>>,
    ) {
