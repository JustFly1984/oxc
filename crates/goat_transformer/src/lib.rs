...
use goat_allocator::{Allocator, TakeIn, Vec as ArenaVec};
use goat_ast::{AstBuilder, ast::*};
use goat_diagnostics::GoatDiagnostic;
use goat_semantic::Scoping;
use goat_span::{GetSpan, SPAN};
use goat_traverse::{ReusableTraverseCtx, Traverse, traverse_mut_with_ctx};
...
pub struct TransformerReturn {
    ...
    pub errors: std::vec::Vec<GoatDiagnostic>,
...
impl<'a> Traverse<'a, TransformState<'a>> for TransformerImpl<'a> {
    ...
    fn enter_decorator(
        &mut self,
        node: &mut goat_ast::ast::Decorator<'a>,
        ctx: &mut TraverseCtx<'a>,
    ) {
