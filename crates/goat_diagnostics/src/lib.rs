...
//!
//! The main type in this module is [`GoatDiagnostic`], which is used by all other oxc tools to
//! report problems. It implements [miette]'s [`Diagnostic`] trait, making it compatible with other
//! tooling you may be using.
//!
//! ```rust,ignore
//! use goat_diagnostics::{GoatDiagnostic, Result};
//! fn my_tool() -> Result<()> {
//!     try_something().map_err(|e| GoatDiagnostic::error(e.to_string()))?;
//!     Ok(())
//! }
//! ```
//!
...
//! use goat_diagnostics::{DiagnosticService, Error, GoatDiagnostic, GraphicalReportHandler, NamedSource};
//!
//! fn my_tool() -> Result<()> {
//!     try_something().map_err(|e| GoatDiagnostic::error(e.to_string()))?;
//!     Ok(())
...
pub type Result<T> = std::result::Result<T, GoatDiagnostic>;
...
#[must_use]
pub struct GoatDiagnostic {
    // `Box` the data to make `GoatDiagnostic` 8 bytes so that `Result` is small.
    // This is required because rust does not performance return value optimization.
    inner: Box<GoatDiagnosticInner>,
}

impl Deref for GoatDiagnostic {
    type Target = Box<GoatDiagnosticInner>;
...
impl DerefMut for GoatDiagnostic {
...
#[derive(Debug, Clone, Eq, PartialEq)]
pub struct GoatDiagnosticInner {
...
impl Display for GoatDiagnostic {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> std::fmt::Result {
        self.message.fmt(f)
    }
}

impl std::error::Error for GoatDiagnostic {}

impl Diagnostic for GoatDiagnostic {
...
impl GoatDiagnostic {
    /// Create new an error-level [`GoatDiagnostic`].
    pub fn error<T: Into<Cow<'static, str>>>(message: T) -> Self {
        Self {
            inner: Box::new(GoatDiagnosticInner {
    ...
    /// Create new a warning-level [`GoatDiagnostic`].
    ...
    ///
    /// Use [`GoatDiagnostic::with_error_code`] to set both the scope and number at once.
    #[inline]
    ...
    ///
    /// Use [`GoatDiagnostic::error`] or [`GoatDiagnostic::warn`] to create a diagnostic at the
    /// severity you want.
    ...
    ///     return Err(GoatDiagnostic::error("No config file found")
    ...
    ///
    /// Existing labels will be removed. Use [`GoatDiagnostic::and_label`] append a label instead.
    ///
    /// You need to add some source code to this diagnostic (using
    /// [`GoatDiagnostic::with_source_code`]) for this to actually be useful. Use
    /// [`GoatDiagnostic::with_labels`] to add multiple labels all at once.
    ///
    /// Note that this pairs nicely with [`goat_span::Span`], particularly the [`label`] method.
    ///
    /// [`goat_span::Span`]: https://docs.rs/goat_span/latest/goat_span/struct.Span.html
    /// [`label`]: https://docs.rs/goat_span/latest/goat_span/struct.Span.html#method.label
    ...
    ///
    /// Existing labels will be removed. Use [`GoatDiagnostic::and_labels`] to append labels
    /// instead.
    ///
    /// You need to add some source code using [`GoatDiagnostic::with_source_code`] for this to
    /// actually be useful. If you only have a single label, consider using
    /// [`GoatDiagnostic::with_label`] instead.
    ///
    /// Note that this pairs nicely with [`goat_span::Span`], particularly the [`label`] method.
    ///
    /// [`goat_span::Span`]: https://docs.rs/goat_span/latest/goat_span/struct.Span.html
    ...
    pub fn inner_owned(self) -> GoatDiagnosticInner {
        *self.inner
    }
}
