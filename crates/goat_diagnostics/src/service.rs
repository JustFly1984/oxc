...
/// use goat_diagnostics::{Error, GoatDiagnostic, DiagnosticService, GraphicalReportHandler};
///
...
///         vec![Error::new(GoatDiagnostic::error("Something went wrong"))],
///     ));
///
        ...
        while let Ok(diagnostics) = self.receiver.recv() {
            ...
            for diagnostic in diagnostics {
                ...
                if let Some(err_str) = self.reporter.render_error(diagnostic) {
                    ...
                    {
                        let mut diagnostic =
                            GoatDiagnostic::warn("File is too long to fit on the screen");
...
mod tests {
    ...
    fn preserves_long_lines_for_single_line_reporters() {
        ...
        sender.send(vec![Error::new(GoatDiagnostic::warn("original diagnostic"))]).unwrap();
        drop(sender);
