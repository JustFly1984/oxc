...
//! ```bash
//! cargo run -p goat_parser --example visitor [filename]
//! ```

use std::{env, path::Path};

use goat_allocator::Allocator;
use goat_ast::ast::{Class, Function, TSImportType};
...
// Instruction:
// create a `test.js`,
// run `cargo run -p goat_parser --example visitor`
// or `cargo watch -x "run -p goat_parser --example visitor"`
