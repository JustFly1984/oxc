...
use goat_allocator::{Allocator, Vec as ArenaVec};
use goat_ast::ast::RegExpFlags;
...
pub struct Lexer<'a, C: Config> {
    ...
    pub(crate) errors: Vec<GoatDiagnostic>,
    ...
    pub(crate) deferred_module_errors: Vec<GoatDiagnostic>,
...
impl<'a, C: Config> Lexer<'a, C> {
    ...
    #[cfg(feature = "benchmarking")]
    pub fn errors(&self) -> &[GoatDiagnostic] {
        &self.errors
    }
    ...
    // ---------- Private Methods ---------- //
    fn error(&mut self, error: GoatDiagnostic) {
