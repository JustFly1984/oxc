...
impl<'a> ModuleRecordBuilder<'a> {
    ...
    pub fn build(mut self) -> (ModuleRecord<'a>, std::vec::Vec<GoatDiagnostic>) {
    ...
    pub fn errors(&self) -> std::vec::Vec<GoatDiagnostic> {
        let mut errors = vec![];
    ...
    fn add_module_request(&mut self, name: Str<'a>, requested_module: RequestedModule) {
        self.module_record
            .requested_modules
            .entry(name)
            .or_insert_with(|| goat_allocator::Vec::new_in(self.allocator))
