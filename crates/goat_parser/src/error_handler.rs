...
impl<'a, C: Config> ParserImpl<'a, C> {
    ...
    /// Push a Syntax Error
    #[cold]
    pub(crate) fn error(&mut self, error: GoatDiagnostic) {
    ...
    #[cold]
    pub(crate) fn error_on_script(&mut self, error: GoatDiagnostic) {
    ...
    #[cold]
    pub(crate) fn set_fatal_error(&mut self, error: GoatDiagnostic) {
    ...
    #[cold]
    pub(crate) fn fatal_error<T: Dummy<'a>>(&mut self, error: GoatDiagnostic) -> T {
        self.set_fatal_error(error);
        Dummy::dummy(self.ast.allocator)
    }
