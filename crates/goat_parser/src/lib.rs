...
//! # Usage
//!
//! The parser has a minimal API with three inputs (a [memory arena](goat_allocator::Allocator), a
...
//! Oxc's AST is located in a separate [`goat_ast`] crate. You can find type definitions for AST
//! nodes [here][`goat_ast::ast`].
//!
//! # Performance
//!
//! The following optimization techniques are used:
//! * AST is allocated in a memory arena ([goat_allocator](https://docs.rs/goat_allocator)) for fast AST drop
//! * [`goat_span::Span`] offsets uses `u32` instead of `usize`
//! * Scope binding, symbol resolution and complicated syntax errors are not done in the parser,
//! they are delegated to the [semantic analyzer](https://docs.rs/goat_semantic)
//!
//! <div class="warning">
//! Because [`goat_span::Span`] uses `u32` instead of `usize`, Oxc can only parse files up
...
//! # Visitor
//!
//! See [`Visit`](http://docs.rs/goat_ast_visit) and [`VisitMut`](http://docs.rs/goat_ast_visit).
//!
...
//! ```
//!
//! See [full linter example](https://github.com/Boshen/oxc/blob/ab2ef4f89ba3ca50c68abb2ca43e36b7793f3673/crates/goat_linter/examples/linter.rs#L38-L39)
...
use goat_allocator::{Allocator, Box as ArenaBox, Dummy, Vec as ArenaVec};
use goat_ast::{
    AstBuilder,
    ast::{Expression, Program},
};
use goat_diagnostics::GoatDiagnostic;
use goat_span::{SourceType, Span};
use goat_syntax::module_record::ModuleRecord;
...
///    enabled](https://docs.rs/goat_semantic/latest/goat_semantic/struct.SemanticBuilder.html#method.with_check_syntax_error)
///
/// ## Errors
...
pub struct ParserReturn<'a> {
    ...
    /// analysis](https://docs.rs/goat_semantic), which can be enabled using
    /// [`SemanticBuilder::with_check_syntax_error`](https://docs.rs/goat_semantic/latest/goat_semantic/struct.SemanticBuilder.html#method.with_check_syntax_error).
    ...
    pub tokens: goat_allocator::Vec<'a, Token>,
...
pub struct ParseOptions {
    ...
    /// Default: `false`
    ///
    /// [`return`]: goat_ast::ast::ReturnStatement
    ...
    /// Default: `true`
    ///
    /// [`ParenthesizedExpression`]: goat_ast::ast::ParenthesizedExpression
    /// [`TSParenthesizedType`]: goat_ast::ast::TSParenthesizedType
    ...
    /// Default: `false`
    ///
    /// [`V8IntrinsicExpression`]: goat_ast::ast::V8IntrinsicExpression
...
impl<'a> Parser<'a> {
    ...
    ///
    /// # Parameters
    /// - `allocator`: [Memory arena](goat_allocator::Allocator) for allocating AST nodes
...
mod parser_parse {
    ...
    impl<'a, C: ParserConfig> Parser<'a, C> {
        ...
        pub fn parse_expression(self) -> Result<Expression<'a>, Vec<GoatDiagnostic>> {
            let unique = UniquePromise::new();
            let parser = ParserImpl::new(
                self.allocator,
                self.source_text,
...
struct ParserImpl<'a, C: ParserConfig> {
    ...
    deferred_script_errors: Vec<GoatDiagnostic>,
...
impl<'a, C: ParserConfig> ParserImpl<'a, C> {
    ...
    pub fn parse_expression(mut self) -> Result<Expression<'a>, Vec<GoatDiagnostic>> {
    ...
    fn reparse_potential_top_level_awaits(
        &mut self,
        statements: &mut goat_allocator::Vec<'a, goat_ast::ast::Statement<'a>>,
    ) {
    ...
    fn flow_error(&mut self) -> Option<GoatDiagnostic> {
    ...
    #[cold]
    fn overlong_error(&self) -> Option<GoatDiagnostic> {
...
mod test {
    ...
    use goat_ast::ast::{CommentKind, Expression, Statement};
    use goat_span::GetSpan;
