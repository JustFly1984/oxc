...
mod test {
    ...
    use goat_ast::ast::{ImportDeclarationSpecifier, ImportOrExportKind, ImportPhase, Statement};
    ...
    fn parse_and_assert_statements(
        ...
        f: fn(Vec<&goat_ast::ast::Statement<'_>>) -> (),
    ) {
    ...
    fn parse_and_assert_import_declarations(
        src: &'static str,
        // takes a function which accepts the list of statements
        f: fn(Vec<&goat_allocator::Box<'_, goat_ast::ast::ImportDeclaration<'_>>>) -> (),
    ) {
        let source_type = SourceType::default().with_typescript(true);
        let allocator = Allocator::default();
        let ret = Parser::new(&allocator, src, source_type).parse();
