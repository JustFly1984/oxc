...
use goat_span::{GetSpan, IdentHashSet, SPAN, SourceType, Str};
...
pub struct IsolatedDeclarations<'a> {
    ...
    // state
    scope: ScopeTree<'a>,
    errors: RefCell<Vec<GoatDiagnostic>>,
...
impl<'a> IsolatedDeclarations<'a> {
    ...
    fn take_errors(&self) -> Vec<GoatDiagnostic> {
        mem::take(&mut self.errors.borrow_mut())
    }

    /// Add an Error
    fn error(&self, error: GoatDiagnostic) {
