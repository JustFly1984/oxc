...
// Read [`macro@goat_ast_macros::ast`] for more information.

use std::{cell::Cell, hash::Hash};

use bitflags::bitflags;
use goat_allocator::{Box, CloneIn, Dummy, TakeIn, UnstableAddress};
use goat_ast_macros::ast;
use goat_estree::ESTree;
use goat_regular_expression::ast::Pattern;
use goat_span::{ContentEq, GetSpan, GetSpanMut, Span, Str};
use goat_syntax::{
    node::NodeId,
...
pub struct RegExpLiteral<'a> {
    ...
    /// Node location in source code.
    pub span: Span,
    /// The parsed regular expression. See [`goat_regular_expression`] for more
    /// details.
    pub regex: RegExp<'a>,
...
pub struct RegExpPattern<'a> {
    ...
    ///
    /// `goat_codegen` ignores `pattern` field, and prints `text`.
...
/// Dummy type to communicate the content of `RegExpFlags` to `goat_ast_tools`.
