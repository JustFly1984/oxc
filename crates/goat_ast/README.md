...
- **Memory efficient**: Designed to work with `goat_allocator` for fast allocation
- **Visitor patterns**: Integrates with `goat_ast_visit` for traversal
...
All AST nodes are allocated in an arena (`goat_allocator`) for optimal performance.
