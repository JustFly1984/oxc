...
impl<'a> SemanticTester<'a> {
    ...
    pub fn build_with_errors(&self) -> SemanticBuilderReturn<'_> {
        let parse =
            goat_parser::Parser::new(&self.allocator, self.source_text, self.source_type).parse();
    ...
    ///
    /// [`parsing`]: goat_parser::Parser::parse
    ...
    fn wrap_diagnostics(&self, diagnostics: Vec<GoatDiagnostic>) -> Vec<Error> {
