...
use goat_semantic::{Reference, ScopeFlags, Semantic, SymbolFlags, SymbolId};
...
/// # Example
/// ```ignore
/// use goat_semantic::{SymbolFlags, Semantic};
...
pub struct SymbolTester<'a> {
    ...
    test_result: Result<SymbolId, GoatDiagnostic>,
}
...
impl<'a> SymbolTester<'a> {
    pub(super) fn new_at_root(
        ...
        let data = decl.map_or_else(
            || Err(GoatDiagnostic::error(format!("Could not find declaration for {target}"))),
            Ok,
        );
    ...
    pub(super) fn new_unique(
        ...
        let data = match symbols_with_target_name.len() {
            0 => Err(GoatDiagnostic::error(format!("Could not find declaration for {target}"))),
            1 => Ok(symbols_with_target_name.pop().unwrap()),
            n if n > 1 => Err(GoatDiagnostic::error(format!(
    ...
    pub(super) fn new_first_binding(
        ...
        let data = match symbols_with_target_name {
            ...
            None => Err(GoatDiagnostic::error(format!("Could not find declaration for {target}"))),
        };
        ...
        self.test_result = match self.test_result {
            Ok(symbol_id) => {
                ...
                if found_flags.contains(flags) {
                    ...
                    Err(GoatDiagnostic::error(format!(
...
impl<'a> Expect<(Rc<Semantic<'a>>, SymbolId), bool> for SymbolTester<'a> {
        ...
        F: FnOnce((Rc<Semantic<'a>>, SymbolId)) -> bool,
    {
        ...
        if did_pass {
            ...
            Self { test_result: Err(GoatDiagnostic::error("Expectation failed")), ..self }
        }
    }
}
...
impl<'a> Expect<(Rc<Semantic<'a>>, SymbolId), Result<(), &'static str>> for SymbolTester<'a> {
        ...
        F: FnOnce((Rc<Semantic<'a>>, SymbolId)) -> Result<(), &'static str>,
    {
        ...
        if let Err(e) = did_pass {
            Self { test_result: Err(GoatDiagnostic::error(e)), ..self }
        } else {
            self
        }
    }
}
impl<'a> Expect<(Rc<Semantic<'a>>, SymbolId), Result<(), GoatDiagnostic>> for SymbolTester<'a> {
    fn expect<'e, F>(self, expectation: F) -> Self
    where
        F: FnOnce((Rc<Semantic<'a>>, SymbolId)) -> Result<(), GoatDiagnostic>,
    {
        let Ok(symbol_id) = self.test_result else { return self };
        let did_pass = expectation((Rc::clone(&self.semantic), symbol_id));
