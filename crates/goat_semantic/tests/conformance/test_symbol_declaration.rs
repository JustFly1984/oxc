...
use goat_syntax::symbol::SymbolId;
...
///
/// [`SymbolTable`]: goat_semantic::SymbolTable
...
fn malformed_binding_pattern(expected_id: SymbolId, pattern: &BindingPattern) -> TestResult {
    GoatDiagnostic::error(format!("BindingPattern for {expected_id:?} is not a destructuring pattern but get_binding_identifier() still returned None"))
        .with_label(pattern.span().label("BindingPattern is here"))
        .into()
}

fn invalid_declaration_node(kind: AstKind) -> TestResult {
    GoatDiagnostic::error(format!("Invalid declaration node kind: {}", kind.debug_name()))
