...
impl SemanticConformance {
    ...
    fn run(&self, semantic: &Semantic) -> Vec<GoatDiagnostic> {
    ...
    fn record_results(
        &self,
        diagnostics: &mut Vec<GoatDiagnostic>,
...
pub enum TestResult {
    Pass,
    Fail(/* reasons */ Vec<GoatDiagnostic>),
}
impl From<String> for TestResult {
    fn from(reason: String) -> Self {
        TestResult::Fail(vec![GoatDiagnostic::error(Cow::Owned(reason))])
    }
}
impl From<Option<String>> for TestResult {
    fn from(result: Option<String>) -> Self {
        match result {
            Some(reason) => TestResult::Fail(vec![GoatDiagnostic::error(Cow::Owned(reason))]),
            None => TestResult::Pass,
        }
    }
}

impl From<GoatDiagnostic> for TestResult {
    fn from(diagnostic: GoatDiagnostic) -> Self {
        TestResult::Fail(vec![diagnostic])
    }
}
impl From<Vec<GoatDiagnostic>> for TestResult {
    fn from(diagnostics: Vec<GoatDiagnostic>) -> Self {
        TestResult::Fail(diagnostics)
    }
}
impl FromIterator<GoatDiagnostic> for TestResult {
    fn from_iter<I: IntoIterator<Item = GoatDiagnostic>>(iter: I) -> Self {
