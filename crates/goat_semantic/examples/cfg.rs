...
//! ```bash
//! cargo run -p goat_semantic --example cfg --features=cfg [filename]
//! ```
//!
//! This generates:
...
use goat_semantic::{SemanticBuilder, dot::DebugDot};
use goat_span::SourceType;

// Instruction:
// 1. create a `test.js`,
// 2. run `cargo run -p goat_semantic --example cfg --features=cfg`
//    or `just watch "run -p goat_semantic --example cfg --features=cfg"`
