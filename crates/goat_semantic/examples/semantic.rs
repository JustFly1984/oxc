...
//! ```bash
//! cargo run -p goat_semantic --example semantic [filename] [--symbols] [--symbol-references]
//! ```
//!
//! ## Options
//!
...
// Instruction:
// create a `test.js`,
// run `cargo run -p goat_semantic --example semantic`
// or `just watch "run -p goat_semantic --example semantic"`
...
fn main() -> std::io::Result<()> {
    ...
    if show_symbol_references {
        ...
        for sym in semantic.semantic.scoping().symbol_ids() {
            ...
            let mut info = GoatDiagnostic::warn(format!("References for symbol `{symbol_name}`"))
